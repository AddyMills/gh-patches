...
script boot_download_scan \{event_params = {
	...
	if (($doing_invite_join = 0) && ($invite_controller != -1))
...
script shutdown_game_for_signin_change \{unloadcontent = 1
		signin_change = 0}
	printf \{channel = ui
		qs("\Lshutdown_game_for_signin_change")}
	...
	printf \{channel = ui
		qs("\Lshutdown_game_for_signin_change end")}
endscript
...
script sysnotify_handle_signin_change 
	...
	sysnotify_wait_until_safe \{semaphore = sysnotify_block_invite
		override = sysnotify_signin_override}
