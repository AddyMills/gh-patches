guitar_hero_cheats = [
	...
	{
		name = unlockall
		name_text = "todas las canciones desbloqueadas"
		var = Cheat_UnlockAll
		unlock_pattern = [
			257
			4112
...
script create_cheats_menu 
	...
	if ($entering_cheat = 0)
		...
		switch <platform>
			case xenon
			warning = "ADVERTENCIA: algunos de los trucos activos no funcionan en el modo Carrera y en línea."
			warning_cont = "Además, el desbloqueo de logros y de marcadores está apagado."
			case ps3
			warning = "ADVERTENCIA: algunos de los trucos activos no funcionan en el modo Carrera y en línea."
			warning_cont = "Además, los marcadores están apagados."
			case Ps2
			warning = "ADVERTENCIA: algunos de los trucos activos no funcionan en el modo Carrera."
			warning_cont = ""
			default
			warning = "ADVERTENCIA: algunos de los trucos activos no funcionan en el modo Carrera y en línea."
			warning_cont = "Además, los marcadores están apagados."
	...
	if (<unlock_Cheat_NoFail> > 0)
		if ($Cheat_NoFail = 1)
			FormatText TextName = text "%c : ACT." c = ($guitar_hero_cheats [3].name_text)
		else
			if ($Cheat_NoFail < 0)
				change \{Cheat_NoFail = 2}
			endif
			FormatText TextName = text "%c : DESACT." c = ($guitar_hero_cheats [3].name_text)
		endif
	endif
	CreateScreenElement {
		<text_params2>
	...
	if (<unlock_Cheat_AirGuitar> > 0)
		if ($Cheat_AirGuitar = 1)
			FormatText TextName = text "%c : ACT." c = ($guitar_hero_cheats [0].name_text)
		else
			if ($Cheat_AirGuitar < 0)
				change \{Cheat_AirGuitar = 2}
			endif
			FormatText TextName = text "%c : DESACT." c = ($guitar_hero_cheats [0].name_text)
		endif
	endif
	CreateScreenElement {
		<text_params2>
	...
	if (<unlock_Cheat_Hyperspeed> > 0)
		if ($Cheat_Hyperspeed > 0)
			FormatText TextName = text "%c : ACT." c = ($guitar_hero_cheats [2].name_text)
			FormatText TextName = text "%c, %d" c = <text> d = ($Cheat_Hyperspeed)
		else
			if ($Cheat_Hyperspeed < 0)
				change \{Cheat_Hyperspeed = 0}
			endif
			FormatText TextName = text "%c : DESACT." c = ($guitar_hero_cheats [2].name_text)
		endif
	endif
	CreateScreenElement {
		<text_params2>
	...
	if (<unlock_Cheat_PerformanceMode> > 0)
		if ($Cheat_PerformanceMode = 1)
			FormatText TextName = text "%c : ACT." c = ($guitar_hero_cheats [1].name_text)
		else
			if ($Cheat_PerformanceMode < 0)
				change \{Cheat_PerformanceMode = 2}
			endif
			FormatText TextName = text "%c : DESACT." c = ($guitar_hero_cheats [1].name_text)
		endif
	endif
	CreateScreenElement {
		<text_params2>
	...
	if (<unlock_Cheat_EasyExpert> > 0)
		if ($Cheat_EasyExpert = 1)
			FormatText TextName = text "%c : ACT." c = ($guitar_hero_cheats [4].name_text)
		else
			if ($Cheat_EasyExpert < 0)
				change \{Cheat_EasyExpert = 2}
			endif
			FormatText TextName = text "%c : DESACT." c = ($guitar_hero_cheats [4].name_text)
		endif
	endif
	CreateScreenElement {
		<text_params2>
	...
	if (<unlock_Cheat_PrecisionMode> > 0)
		if ($Cheat_PrecisionMode = 1)
			FormatText TextName = text "%c : ACT." c = ($guitar_hero_cheats [5].name_text)
		else
			if ($Cheat_PrecisionMode < 0)
				change \{Cheat_PrecisionMode = 2}
			endif
			FormatText TextName = text "%c : DESACT." c = ($guitar_hero_cheats [5].name_text)
		endif
	endif
	CreateScreenElement {
		<text_params2>
	...
	if (<unlock_Cheat_BretMichaels> > 0)
		if ($Cheat_BretMichaels = 1)
			FormatText TextName = text "%c : ACT." c = ($guitar_hero_cheats [6].name_text)
		else
			if ($Cheat_BretMichaels < 0)
				change \{Cheat_BretMichaels = 2}
			endif
			FormatText TextName = text "%c : DESACT." c = ($guitar_hero_cheats [6].name_text)
		endif
	endif
	CreateScreenElement {
		<text_params2>
...
script cheats_zoomin_guitar 
	...
	add_user_control_helper \{text = "ACABAR DE INTRODUCIR TRUCO"
		button = start
		z = 100
		fit_to_rectangle = 0}
	add_user_control_helper \{text = "ARRIBA/ABAJO PARA RASGUEAR"
		button = strumbar
		z = 100
		fit_to_rectangle = 0}
...
script toggle_hyperspeed 
	if ($<cheat> >= 0)
		if ($<cheat> = 5)
			...
			FormatText TextName = text "%c : DESACT." c = ($guitar_hero_cheats [<index>].name_text)
			SetScreenElementProps id = <id> text = <text>
		else
			change globalname = <cheat> newvalue = ($<cheat> + 1)
			FormatText TextName = text "%c : ACT." c = ($guitar_hero_cheats [<index>].name_text)
			FormatText TextName = text "%c, %d" c = <text> d = ($Cheat_Hyperspeed)
			SetScreenElementProps id = <id> text = <text>
		endif
	endif
...
script toggle_cheat 
	if ($<cheat> > 0)
		...
		SetScreenElementProps id = <id> text = "bloqueado"
	endif
	show_cheat_warning
endscript
