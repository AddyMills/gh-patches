np_5star_headlines = [
	"EL PÚBLICO, VÍCTIMA DE UNA DESCARGA DE VATIOS DE %b"
	"%b ARRASA EN TODO EL MUNDO CON SU IMPRESIONANTE ESPECTÁCULO"
	"10 DE CADA 10 ROCKEROS ADORAN A %b"
	"EL TERROR DE LAS CHICAS. HA LLEGADO %b"
]
num_5star_headlines = 4
np_4star_headlines = [
	"%b IMPRESIONA AL PÚBLICO CON UNA BUENA ACTUACIÓN"
	"LA TERCERA EDAD COINCIDE POR UNANIMIDAD: %B ES DEMASIADO POTENTE"
	"EL PÚBLICO SE RINDE AL GRAN ESPECTÁCULO DE %b"
]
num_4star_headlines = 3
np_3star_headlines = [
	"%b NO LO HACE TAN REMATADAMENTE MAL"
	"EL PÚBLICO MUESTRA UN TÍMIDO INTERÉS POR %b"
	"%b SUENA MEJOR QUE UNAS UÑAS ARAÑANDO UNA PIZARRA"
]
...
script create_newspaper_menu \{for_practice = 0}
	...
	if NOT ($game_mode = p1_career || $game_mode = p2_career || $game_mode = p2_coop || $game_mode = p1_quickplay || <for_practice> = 1)
		...
		if ($game_mode = p2_battle)
			...
			FormatText TextName = winner_text "Jugador %d" d = <winner>
			if ($is_network_game)
				if (<winner> = "2")
			...
			FormatText TextName = who_won_text "¡Reglas de %s!" s = <winner_text>
			displayText {
				parent = newspaper_container
				text = <who_won_text>
				just = [left top]
			...
			if (<winner> = "0")
				...
				FormatText TextName = who_won_text "¡%d te supera con creces!" d = <winner_text>
			endif
			CreateScreenElement {
				type = TextBlockElement
				parent = newspaper_container
		...
		if (<for_practice> = 0)
			...
			FormatText TextName = notes_hit_out_of_total "%a DE %b" a = <notes_hit> b = <notes_total>
			displayText {
				id = np_score_text
				parent = newspaper_container
	...
	if ($is_network_game = 1)
		add_user_control_helper \{text = "CONTINÚA"
			button = green
			z = 100}
	else
...
script np_create_options_menu \{pos = (600.0, 300.0)
	...
	if (<for_practice> = 1)
		displayText id = np_option_0 parent = newspaper_container text = "CONTINÚA" pos = (($g_np_option_props [4].pos) + <menu_offset>) scale = (0.85, 0.7) rot = ($g_np_option_props [4].rot) font = <menu_font> noshadow
		displayText id = np_option_1 parent = newspaper_container text = "RECOMENZAR" pos = (($g_np_option_props [5].pos) + <menu_offset>) scale = (0.8, 0.7) rot = ($g_np_option_props [5].rot) font = <menu_font> noshadow
		displayText id = np_option_2 parent = newspaper_container text = "CAMBIAR VELOC." pos = (($g_np_option_props [6].pos) + <menu_offset>) scale = (0.8, 0.7) rot = ($g_np_option_props [6].rot) font = <menu_font> noshadow
		displayText id = np_option_3 parent = newspaper_container text = "CAMBIAR SECCIÓN" pos = (($g_np_option_props [7].pos) + <menu_offset>) scale = (0.8, 0.7) rot = ($g_np_option_props [7].rot) font = <menu_font> noshadow
		displayText id = np_option_4 parent = newspaper_container text = "SALIR" pos = (($g_np_option_props [8].pos) + <menu_offset>) scale = (0.8, 0.7) rot = ($g_np_option_props [8].rot) font = <menu_font> noshadow
		retail_menu_unfocus \{id = np_option_4}
		<initial_hl_pos> = (($g_np_option_props [4].pos) + ($g_np_option_props [4].offset) + <menu_offset>)
	else
		displayText id = np_option_0 parent = newspaper_container text = "CONTINÚA" pos = (($g_np_option_props [0].pos) + <menu_offset>) scale = (0.85, 0.7) rot = ($g_np_option_props [0].rot) font = <menu_font> noshadow
		SetScreenElementProps id = <id> font_spacing = 2 space_spacing = 4
		if NOT ($end_credits = 1)
			if (<show_replay> = 1)
				displayText id = np_option_1 parent = newspaper_container text = "REINTENTAR" pos = (($g_np_option_props [1].pos) + <menu_offset>) scale = (0.8, 0.7) rot = ($g_np_option_props [1].rot) font = <menu_font> noshadow
				SetScreenElementProps id = <id> font_spacing = 2 space_spacing = 4
				displayText id = np_option_2 parent = newspaper_container text = "ESTADÍSTICAS" pos = (($g_np_option_props [2].pos) + <menu_offset>) scale = (0.8, 0.7) rot = ($g_np_option_props [2].rot) font = <menu_font> noshadow
				SetScreenElementProps id = <id> font_spacing = 2 space_spacing = 4
			else
				displayText id = np_option_1 parent = newspaper_container text = "ESTADÍSTICAS" pos = (($g_np_option_props [1].pos) + <menu_offset>) scale = (0.8, 0.7) rot = ($g_np_option_props [2].rot) font = <menu_font> noshadow
				SetScreenElementProps id = <id> font_spacing = 2 space_spacing = 4
			endif
		endif
