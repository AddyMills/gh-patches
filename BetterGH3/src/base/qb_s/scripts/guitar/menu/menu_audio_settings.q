...
script create_audio_settings_menu \{popup = 0}
	...
	if (<popup> = 1)
		...
		if NOT isSinglePlayerGame
			FormatText TextName = player_paused_text "JUGADOR %d EN PAUSA. SOLO OPCIONES DEL JUGADOR %d DISPONIBLES." d = (($last_start_pressed_device) + 1)
			displaySprite {
	...
	if (<popup>)
		FormatText TextName = text "GRUPO: %d" d = <band_volume>
	...
	if (<popup>)
		FormatText TextName = text "GUITARRA: %d" d = <guitar_volume>
		<exclusive_params> = {exclusive_device = ($last_start_pressed_device)}
		displaySprite \{parent = pause_menu_frame_container
