...
script xbox360_live_not_allowed 
	...
	if isXenon
		...
		if GotParam \{age_restricted}
			...
			<sign_in_text> = "Deber iniciar sesión para acceder al lobby."
		endif
	endif
	scale = 1
...
script invite_accepted 
	...
	if ((($is_network_game = 0) && ($playing_song)) &&
		...
		create_popup_warning_menu {
			textblock = {
				text = "¿Seguro que quieres salir de esta sesión?"
				pos = (640.0, 380.0)
			}
...
script net_dl_content_compatabilty_warning \{z = 100
	...
	text = "Los dos jugadores tienen que tener las mismas canciones descargadas para poder usarlas en partidas multijugador."
	CreateScreenElement {
		type = TextBlockElement
		parent = <menu_anchor>
		font = text_a4
