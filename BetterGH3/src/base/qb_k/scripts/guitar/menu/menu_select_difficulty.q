...
script create_mp_select_difficulty_menu 
	...
	if ($is_network_game = 1)
		...
		displayText parent = sd_container text = "플레이어" font = <menu_font> pos = (260.0, 160.0) scale = 1 rgba = [205 105 50 255] noshadow
		GetScreenElementDims id = <id>
		if (<width> > 150)
			SetScreenElementProps id = <id> scale = 1
			fit_text_in_rectangle id = <id> dims = ((150.0, 0.0) + ((0.0, 1.0) * <height>)) pos = (260.0, 160.0)
		endif
		displayText parent = sd_container text = "플레이어" font = <menu_font> pos = (860.0, 160.0) scale = 1 rgba = [205 105 50 255] noshadow
		GetScreenElementDims id = <id>
		if (<width> > 150)
			SetScreenElementProps id = <id> scale = 1
	...
	if ($game_mode = p2_career)
		...
		displayText \{parent = sd_container
			...
			text = "겁쟁이처럼 굴지마십시오! 돈과 잠금 해제 정도에 따라 최저 난이도가 결정됩니다!"
			rgba = [
				225
				120
				60
