...
online_menu_fs = {
	...
	actions = [
		...
		{
			action = select_join_private
			flow_state = join_private_match_fs
		}
		{
		...
		{
			action = select_account_management
...
private_match_search_fs = {
	...
	actions = [
		{
			action = select_server
			flow_state = private_match_joining_game_fs
		}
		{
			action = response_create_selected
			flow_state_func = select_create_match_transition
...
online_winport_start_connection_fs = {
	...
	actions = [
		...
		{
			action = back
			flow_state_func = whats_before_online_menu
		}
	]
}
...
join_private_match_fs = {
	...
	actions = [
		{
			action = attempt_join
			flow_state = private_match_search_fs
		}
		{
			action = back
			flow_state = online_menu_fs
