...
script create_detailed_stats_menu 
	...
	if ($game_mode = p2_battle)
		p1_sp_ratio = "없음"
		p1_avg_multiplier = "없음"
		p1_lead_percent = "없음"
		p2_sp_ratio = "없음"
		p2_avg_multiplier = "없음"
		p2_lead_percent = "없음"
	endif
	left_margin = 400
	right_margin = 800
...
script add_basic_stats_desc 
	...
	add_text_to_column column = <desc_column> text = "노트 성공" dims = (400.0, 0.0)
	if NOT ($game_mode = p2_battle || $current_song = bosstom || $current_song = bossslash || $current_song = bossdevil)
		add_text_to_column column = <desc_column> text = "SP 악구" dims = (400.0, 0.0)
		add_text_to_column column = <desc_column> text = "평균 멀티플라이어" dims = (400.0, 0.0)
		if ($current_num_players = 2)
			if ($game_mode = p2_faceoff ||
					$game_mode = p2_pro_faceoff ||
					$game_mode = p2_battle)
				add_text_to_column column = <desc_column> text = "리드한 시간" dims = (400.0, 0.0)
			endif
		endif
	else
		add_text_to_column column = <desc_column> text = "사용한 어택" dims = (400.0, 0.0)
	endif
endscript
