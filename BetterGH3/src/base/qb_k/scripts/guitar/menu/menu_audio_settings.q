...
script create_audio_settings_menu \{popup = 0}
	...
	if (<popup> = 1)
		...
		if NOT issingleplayergame
			FormatText textname = player_paused_text "플레이어 %d님이 일시 중지했습니다. 플레이어 %d 옵션만 사용할 수 있습니다." d = (($last_start_pressed_device) + 1)
			displaySprite {
	...
	if (<popup>)
		FormatText textname = text "밴드: %d" d = <band_volume>
	...
	if (<popup>)
		FormatText textname = text "기타: %d" d = <guitar_volume>
		<exclusive_params> = {exclusive_device = ($last_start_pressed_device)}
		displaySprite \{parent = pause_menu_frame_container
	...
	if (<popup>)
		FormatText textname = text "효과음: %d" d = <sfx_volume>
		<exclusive_params> = {exclusive_device = ($last_start_pressed_device)}
		displaySprite \{parent = pause_menu_frame_container
			tex = HUD_score_nixie_2a
			rgba = [
