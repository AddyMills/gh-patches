...
script create_main_menu_backdrop 
	...
	FormatText textname = version_string_display "버전 %s" s = <version_string>
...
script create_pause_menu \{player = 1
	...
	if ($is_network_game = 0)
		...
		if GotParam \{banner_text}
			...
			if (<for_options> = 0)
				if (<for_practice> = 1)
					<pause_player_text> = "일시 중지"
				else
					if NOT issingleplayergame
						FormatText textname = pause_player_text "P%d 일시 중지" d = <player>
					else
						<pause_player_text> = "일시 중지"
					endif
				endif
	...
	if (<for_options> = 0)
		...
		if issingleplayergame
			lefty_flip_text = "왼손잡이:"
		else
			if (<player> = 1)
				lefty_flip_text = "P1 왼손잡이:"
			else
				lefty_flip_text = "P2 왼손잡이:"
			endif
		endif
		CreateScreenElement {
			type = ContainerElement
	...
	if ($is_network_game = 0)
		if NOT issingleplayergame
			if (<for_practice> = 0)
				FormatText textname = player_paused_text "플레이어 %d님이 일시 중지했습니다. 플레이어 %d 옵션만 사용할 수 있습니다." d = <player>
				displaySprite {
					parent = pause_menu_frame_container
...
script get_diff_completion_text \{for_p2_career = 0}
	...
	if NOT (<for_p2_career>)
		FormatText textname = diff_completion_string "노래 %b곡 중 %a곡" a = <diff_songs_completed> b = <diff_num_songs>
		SetArrayElement ArrayName = diff_completion_text index = (<diff_index>) newvalue = (<diff_completion_string>)
	else
		FormatText textname = diff_completion_string "노래 %b곡 중 %a곡 완료" a = <diff_songs_completed> b = <diff_num_songs>
		SetArrayElement ArrayName = diff_completion_text index = (<diff_index>) newvalue = (<diff_completion_string>)
	endif
	progression_push_current
...
endscript
winport_confirm_exit_msg = "종료하시겠습니까?"
