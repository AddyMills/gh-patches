...
script create_main_menu_backdrop 
	...
	FormatText TextName = version_string_display "VERSIONE %s" s = <version_string>
...
script pop_in_new_downloads_notifier \{time = 0.5}
	...
	pos = (100.0, 390.0)
	text = "NUOVI CONTENUTI SCARICABILI!"
	CreateScreenElement {
		type = TextElement
		parent = main_menu_text_container
		text = <text>
...
script create_pause_menu \{player = 1
	...
	if ($is_network_game = 0)
		...
		if GotParam \{banner_text}
			...
			if (<for_options> = 0)
				if (<for_practice> = 1)
					<pause_player_text> = "IN PAUSA"
				else
					if NOT isSinglePlayerGame
						FormatText TextName = pause_player_text "G%d-PAUSA" d = <player>
					else
						<pause_player_text> = "IN PAUSA"
					endif
				endif
	...
	if (<for_options> = 0)
		...
		if isSinglePlayerGame
			lefty_flip_text = "GIOCATORE MANCINO:"
		else
			if (<player> = 1)
				lefty_flip_text = "G1 MANCINO:"
			else
				lefty_flip_text = "G2 MANCINO:"
			endif
		endif
		CreateScreenElement {
			type = ContainerElement
	...
	if ($is_network_game = 0)
		if NOT isSinglePlayerGame
			if (<for_practice> = 0)
				FormatText TextName = player_paused_text "GIOC. %d IN PAUSA. SONO DISPONIBILI SOLO LE OPZIONI PER IL GIOC. %d." d = <player>
				displaySprite {
					parent = pause_menu_frame_container
...
script get_diff_completion_text \{for_p2_career = 0}
	...
	if NOT (<for_p2_career>)
		FormatText TextName = diff_completion_string "%a CANZONI SU %b" a = <diff_songs_completed> b = <diff_num_songs>
		SetArrayElement ArrayName = diff_completion_text index = (<diff_index>) newvalue = (<diff_completion_string>)
	else
		FormatText TextName = diff_completion_string "%a di %b brani completati" a = <diff_songs_completed> b = <diff_num_songs>
		SetArrayElement ArrayName = diff_completion_text index = (<diff_index>) newvalue = (<diff_completion_string>)
	endif
	progression_push_current
...
endscript
winport_confirm_exit_msg = "Vuoi uscire?"
