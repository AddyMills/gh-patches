...
script np_net_create_options_menu \{pos = (600.0, 300.0)
	...
	displayText id = np_option_0 parent = newspaper_container text = "CONTINUA" pos = (($g_np_option_props [0].pos) + <menu_offset> + (0.0, 13.0)) scale = (0.85, 0.7) rot = ($g_np_option_props [0].rot) font = <menu_font> noshadow
...
script do_tie_breaker_selection 
	...
	LaunchEvent \{type = unfocus
		target = newspaper_vmenu}
	FormatText TextName = final_song_text "Il punteggio è %d-%d.  Il brano di spareggio verrà scelto da..." d = (<num_songs> / 2)
