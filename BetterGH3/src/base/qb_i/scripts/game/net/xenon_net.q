...
script translate_server_checksums_to_strings 
	...
	if GotParam \{game_mode_checksum}
		switch (<game_mode_checksum>)
			case p2_battle
			<game_mode_value> = "BATTAGLIA"
			case p2_faceoff
			<game_mode_value> = "SFIDA"
			case p2_pro_faceoff
			<game_mode_value> = "SFIDA PRO"
			if GotParam \{difficulty_checksum}
				switch (<difficulty_checksum>)
					case easy
					<game_mode_value> = "SFIDA PRO - FACILE"
					case medium
					<game_mode_value> = "SFIDA PRO - MEDIO"
					case hard
					<game_mode_value> = "SFIDA PRO - DIFFICILE"
					case expert
					<game_mode_value> = "SFIDA PRO ESPERTI"
				endswitch
			endif
			case p2_coop
			<game_mode_value> = "COOPERATIVA"
		endswitch
	endif
