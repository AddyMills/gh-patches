...
script pop_in_new_downloads_notifier \{time = 0.5}
	...
	pos = (100.0, 390.0)
	text = "NOUVEAU  CONTENU  TÉLÉCHARGEABLE !"
	CreateScreenElement {
		type = TextElement
		parent = main_menu_text_container
		text = <text>
...
script create_pause_menu \{player = 1
	...
	if ($is_network_game = 0)
		...
		if GotParam \{banner_text}
			...
			if (<for_options> = 0)
				if (<for_practice> = 1)
					<pause_player_text> = "EN PAUSE"
				else
					if NOT isSinglePlayerGame
						FormatText TextName = pause_player_text "PAUSE J%d " d = <player>
					else
						<pause_player_text> = "EN PAUSE"
					endif
				endif
	...
	if (<for_options> = 0)
		...
		if isSinglePlayerGame
			lefty_flip_text = "GAUCHER :"
		else
			if (<player> = 1)
				lefty_flip_text = "GAUCHER J1 :"
			else
				lefty_flip_text = "GAUCHER J2 :"
			endif
		endif
		CreateScreenElement {
			type = ContainerElement
	...
	if ($is_network_game = 0)
		if NOT isSinglePlayerGame
			if (<for_practice> = 0)
				FormatText TextName = player_paused_text "%d. SEULES LES OPTIONS DU JOUEUR %d SONT DISPONIBLES." d = <player>
				displaySprite {
					parent = pause_menu_frame_container
...
script get_diff_completion_text \{for_p2_career = 0}
	...
	if NOT (<for_p2_career>)
		FormatText TextName = diff_completion_string "%a SUR %b CHANSONS " a = <diff_songs_completed> b = <diff_num_songs>
		SetArrayElement ArrayName = diff_completion_text index = (<diff_index>) newvalue = (<diff_completion_string>)
	else
		FormatText TextName = diff_completion_string "%a sur %b chansons complétées" a = <diff_songs_completed> b = <diff_num_songs>
		SetArrayElement ArrayName = diff_completion_text index = (<diff_index>) newvalue = (<diff_completion_string>)
	endif
	progression_push_current
...
endscript
winport_confirm_exit_msg = "Veux-tu vraiment quitter ?"
