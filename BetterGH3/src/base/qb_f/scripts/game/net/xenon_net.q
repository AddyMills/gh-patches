...
script translate_server_checksums_to_strings 
	...
	if GotParam \{game_mode_checksum}
		switch (<game_mode_checksum>)
			case p2_battle
			<game_mode_value> = "CHOC DES GUITARES"
			case p2_faceoff
			...
			if GotParam \{difficulty_checksum}
				switch (<difficulty_checksum>)
					case easy
					<game_mode_value> = "DUEL PRO - FACILE"
					case medium
					<game_mode_value> = "DUEL PRO - MOYEN"
					case hard
					<game_mode_value> = "DUEL PRO - DIFFICILE"
					case expert
					<game_mode_value> = "DUEL PRO EXPERT"
				endswitch
			endif
			case p2_coop
