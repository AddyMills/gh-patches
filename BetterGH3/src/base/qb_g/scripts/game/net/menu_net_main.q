info_text = [
	"Spring ins Spielerduell und hau gegen den erstbesten Rocker in die Saiten."
	"Starte direkt in einem Ranglisten-Spiel und rock das Haus gegen den ersten verfügbaren Rocker."
	"Bestimmtes Online-Spiel suchen."
	"Nimm an einem von einem Freund erstellten privaten Spiel teil."
	"Erstelle und hoste dein eigenes Online-Spiel."
	"Optionen für Schnelles Spiel festlegen und ändern."
	"Sieh nach, wer die absoluten Spitzenreiter sind und finde heraus, wo du stehst."
	"Downloads kommen bald!"
	"Wähle 'Nachricht des Tages', um die aktuellen Guitar Hero-Neuigkeiten zu erfahren."
	"Verwalte dein Online-Konto."
]
online_main_menu_pos = (470.0, 110.0)
online_info_pane_pos = (890.0, 150.0)

script create_online_main_menu \{menu_title_xenon = "Online-Hauptmenü"
		menu_title_ps3 = "Online-Hauptmenü"
	...
	net_add_item_to_main_menu {
		VMenu = <vmenu_id>
		text = "Optionen für Schnelles Spiel"
		info_text_index = 5
	...
	if NOT isXenon
		if NOT NetSessionFunc \{obj = voice
			...
			CreateScreenElement {
				...
				pos = (640.0, 585.0)
				scale = (0.55, 0.55)
				text = "Die Chat-Funktion für Ihr PLAYSTATION®Network Konto ist aufgrund der Kindersicherung deaktiviert."
				font = text_a4
				rgba = ($online_light_blue)
				z_priority = 6.0
				dims = (1500.0, 120.0)
...
script create_link_text 
	...
	CreateScreenElement {
		...
		scale = (0.75, 0.75)
		rgba = ($online_light_blue)
		text = "Bist du bereit, ein Guitar Hero zu sein? \\nSo verlinkst du deine Statistiken:"
		just = [center top]
		internal_just = [center top]
		z_priority = 6.0
		pos = (640.0, 160.0)
	...
	CreateScreenElement {
		...
		scale = (0.75, 0.75)
		rgba = ($online_light_blue)
		text = "- Besuche die Website www.guitarhero.com\\n- Erstelle ein neues Konto oder melde dich an\\n- Klicke auf Konto verbinden\\n- Gib den folgenden VIP-Passcode ein"
		just = [center top]
		internal_just = [left top]
		z_priority = 6.0
		pos = (640.0, 240.0)
	...
	CreateScreenElement {
		...
		scale = (0.75, 0.75)
		rgba = ($online_light_blue)
		text = "Im Internet kannst du dein Profil personalisieren, die Bestenlisten durchsehen, mit einer Online-Band jammen, Groupies aufreißen und in Wettkämpfen abrocken!"
		just = [center top]
		internal_just = [left top]
		z_priority = 6.0
		pos = (648.0, 460.0)
...
script net_add_item_to_main_menu \{line_spacing = 40}
	...
	if (<text> = "Schnelles Spiel: Ranglisten-Spiel")
