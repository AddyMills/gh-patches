...
script create_winport_connection_status_screen 
	...
	if NOT (NetSessionFunc func = IsConnected)
		add_user_control_helper \{text = "ABBRECHEN"
			button = red
			z = 100}
		...
		switch (<CurrentNetworkTask>)
			...
			switch (<CurrentNetworkStatus>)
				case "PENDING"
				statusText = "Online-Dienste werden initialisiert."
				case "DONE"
				statusText = "Online-Dienste bereit."
				case "FAILED"
				statusText = "Online-Dienste können nicht initialisiert werden."
				success = false
				default
				statusText = "Interner Fehler: Unerwarteter Netzwerkzustand!"
				success = false
			endswitch
			case "CHECK_DNS"
			switch (<CurrentNetworkStatus>)
				case "PENDING"
				statusText = "Spielserver werden gesucht."
				case "DONE"
				statusText = "Spielserver gefunden."
				success = true
				case "FAILED"
				statusText = "Keine Spielserver gefunden."
				success = false
				default
				statusText = "Interner Fehler: Unerwarteter Netzwerkzustand!"
				success = false
	...
	if NOT (NetSessionFunc func = HasExistingLogin)
		SetScreenElementProps \{id = statusMessage
			text = "Vorhandener Anmeldename nicht gefunden.\\nMöchtest du ein neues Konto erstellen oder ein vorhandenes Konto verwenden?"}
		fit_text_into_menu_item \{id = statusMessage
			max_width = 480}
		displaySprite \{parent = connectionStatusContainer
			id = options_bg_1
...
script create_winport_account_login_screen 
	...
	if (<autoLoginSetting> = autoLoginOn && NetSessionFunc func = HasExistingLogin)
		...
		create_winport_account_management_screen \{mode = loginAccount
			...
			yellowButtonText = "KENNWORT ÄNDERN"
			yellowButtonAction = start_winport_account_change_screen
			blueButtonText = "NEUES KONTO"
...
script create_winport_account_change_screen 
	create_winport_account_management_screen \{mode = changeAccount
		...
		yellowButtonText = "KENNWORT ZURÜCKSETZEN"
...
script create_winport_account_reset_screen 
	create_winport_account_management_screen \{mode = resetAccount
		...
		yellowButtonText = "KONTO LÖSCHEN"
...
script create_winport_account_management_screen 
	...
	CreateScreenElement {
		...
		scale = 0.6
		rgba = [255 0 0 255]
		text = "(Feststelltaste ist aktiviert)"
		just = [center top]
		z_priority = 10.0
		pos = (640.0, 530.0)
		shadow
	...
	CreateScreenElement {
		...
		text = "Die Spielerfahrung kann sich beim Online-Spiel ändern."
		just = [center top]
		z_priority = 10.0
		pos = (640.0, 560.0)
		shadow
	...
	CreateScreenElement {
		...
		scale = 1.0
		rgba = [180 180 180 255]
		text = "*Bitte gib alle Kontoinformationen über die Tastatur ein.*"
		just = [center top]
		z_priority = 10.0
		pos = (640.0, 595.0)
		shadow
	...
	<pos> = (375.0, 290.0)
	create_winport_login_field container = <container> pos = <pos> label = "Benutzername: " labelId = usernameLabelId prefixId = usernamePrefixId cursorId = usernameCursorId suffixId = usernameSuffixId ang = -2.0
	...
	if (<mode> = loginAccount || <mode> = deleteAccount || <mode> = changeAccount)
		...
		create_winport_login_field container = <container> pos = <pos> label = "Kennwort: " labelId = passwordLabelId prefixId = passwordPrefixId cursorId = passwordCursorId suffixId = passwordSuffixId ang = 0.2
	endif
	if (<mode> = createAccount || <mode> = changeAccount || <mode> = resetAccount)
		pos = (<pos> + ((0.0, 1.0) * <lineHeight>))
		create_winport_login_field container = <container> pos = <pos> label = "Neues Kennwort: " labelId = newPassword1LabelId prefixId = newPassword1PrefixId cursorId = newPassword1CursorId suffixId = newPassword1SuffixId ang = -0.6
		pos = (<pos> + ((0.0, 1.0) * <lineHeight>))
		create_winport_login_field container = <container> pos = <pos> label = "Neues Kennwort erneut eingeben: " labelId = newPassword2LabelId prefixId = newPassword2PrefixId cursorId = newPassword2CursorId suffixId = newPassword2SuffixId ang = 0.5
	endif
	if (<mode> = createAccount || <mode> = resetAccount)
		pos = (<pos> + ((0.0, 1.0) * <lineHeight>))
		create_winport_login_field container = <container> pos = <pos> label = "Lizenz: " labelId = licenseLabelId prefixId = licensePrefixId cursorId = licenseCursorId suffixId = licenseSuffixId ang = 1.5
	endif
	add_user_control_helper \{text = "ANNEHMEN"
		button = green
		z = 100}
	add_user_control_helper \{text = "ZURÜCK"
		button = red
...
script create_winport_account_management_status_screen 
	...
	switch (<CurrentNetworkTask>)
		...
		switch (<CurrentNetworkStatus>)
			case "PENDING"
			statusText = "Kontoerstellung wird angefordert."
			case "DONE"
			statusText = "Konto erstellt."
			success = true
			case "FAILED"
			statusText = "Konto kann nicht erstellt werden."
			success = false
			default
			statusText = "Interner Fehler: Unerwarteter Netzwerkzustand!"
			success = false
		...
		switch (<CurrentNetworkStatus>)
			case "PENDING"
			statusText = "Konto wird autorisiert."
			case "DONE"
			statusText = "Konto autorisiert."
			success = true
			case "FAILED"
			statusText = "Konto kann nicht autorisiert werden."
			success = false
			default
			statusText = "Interner Fehler: Unerwarteter Netzwerkzustand!"
			success = false
		...
		switch (<CurrentNetworkStatus>)
			case "PENDING"
			statusText = "Kennwortänderung wird angefordert."
			case "DONE"
			statusText = "Kennwort geändert."
			success = true
			case "FAILED"
			statusText = "Kennwort kann nicht geändert werden."
			success = false
			default
			statusText = "Interner Fehler: Unerwarteter Netzwerkzustand!"
			success = false
		...
		switch (<CurrentNetworkStatus>)
			case "PENDING"
			statusText = "Kontozurücksetzung wird angefordert."
			case "DONE"
			statusText = "Kontokennwort zurückgesetzt."
			success = true
			case "FAILED"
			statusText = "Konto kann nicht zurückgesetzt werden."
			success = false
			default
			statusText = "Interner Fehler: Unerwarteter Netzwerkzustand!"
			success = false
		...
		switch (<CurrentNetworkStatus>)
			case "PENDING"
			statusText = "Kontolöschung wird angefordert."
			case "DONE"
			statusText = "Konto gelöscht."
			success = true
			case "FAILED"
			statusText = "Konto kann nicht gelöscht werden."
			success = false
			default
			statusText = "Interner Fehler: Unerwarteter Netzwerkzustand!"
			success = false
	...
	if (<success> = false)
		...
		switch <FailureCode>
			case 666
			statusText = "Neue Kennwörter stimmen nicht überein."
			case 667
			statusText = "Autorisierungsdienst fehlgeschlagen."
			case 668
			statusText = "Benutzername muss 6 bis 16 Zeichen lang sein."
			case 669
			statusText = "Kennwort muss 6 bis 16 Zeichen lang sein"
			case 700
			statusText = "Task erfolgreich abgeschlossen."
			case 701
			statusText = "Ungültige Autorisierungsanforderung."
			case 702
			statusText = "Serverkonfigurationsfehler"
			case 703
			statusText = "Ungültige Spieltitelkennung."
			case 704
			statusText = "Ungültige Kontoinformationen."
			case 705
			statusText = "Unzulässige Autorisierungsanforderung."
			case 706
			statusText = "Ungültiger Lizenzcode."
			case 707
			statusText = "Benutzername bereits vorhanden."
			case 708
			statusText = "Ungültiges Benutzernamenformat."
			case 709
			statusText = "Benutzername abgelehnt."
			case 710
			statusText = "Zu viele Konten für Lizenzcode."
			case 711
			statusText = "Kontomigration wird nicht unterstützt."
			case 712
			statusText = "Titel wurde deaktiviert."
			case 713
			statusText = "Konto ist abgelaufen."
			case 714
			statusText = "Konto ist gesperrt."
			case 715
			statusText = "Berechtigungsfehler: Die Online-Funktionen sind nicht verfügbar, bis Guitar Hero III beendet und neu gestartet wird."
			case 716
			statusText = "Ungültiges Kennwort."
		endswitch
		SetScreenElementProps id = statusMessage text = <statusText>
		fit_text_into_menu_item \{id = statusMessage
			max_width = 480}
...
script create_join_private_menu 
	...
	CreateScreenElement \{type = TextElement
		...
		text = "An privatem Spiel teilnehmen"
		just = [
			center
			top
		]
	...
	CreateScreenElement \{type = TextElement
		...
		text = "*Gib über die Tastatur den Benutzernamen des Spiels ein, an dem du teilnehmen möchtest*"
		just = [
			center
			top
		]
	...
	<pos> = (375.0, 320.0)
	create_winport_login_field container = private_menu_container pos = <pos> label = "Benutzername des Spiels: " labelId = usernameLabelId prefixId = usernamePrefixId cursorId = usernameCursorId suffixId = usernameSuffixId ang = -2.0
	add_user_control_helper \{text = "ANNEHMEN"
		button = green
		z = 100}
	add_user_control_helper \{text = "ZURÜCK"
...
script create_logout_submenu 
	...
	CreateScreenElement {
		...
		parent = logoutContainer
		id = statusMessage
		text = "Die Wahl von Abmeldung beendet deine Online-Sitzung. Deine Punkte werden nicht auf der Bestenliste verfolgt, bis du dich wieder anmeldest."
		font = text_a4
		scale = 0.8
		rgba = [210 210 210 250]
		just = [center top]
...
script create_account_submenu \{menu_title = "KONTOVERWALTUNG"
