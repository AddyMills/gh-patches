...
script create_mp_select_difficulty_menu 
	...
	if ($is_network_game = 1)
		...
		displayText parent = sd_container text = "Spieler   " font = <menu_font> pos = (260.0, 160.0) scale = 1 rgba = [205 105 50 255] noshadow
		GetScreenElementDims id = <id>
		if (<width> > 150)
			SetScreenElementProps id = <id> scale = 1
			fit_text_in_rectangle id = <id> dims = ((150.0, 0.0) + ((0.0, 1.0) * <Height>)) pos = (260.0, 160.0)
		endif
		displayText parent = sd_container text = "Spieler   " font = <menu_font> pos = (860.0, 160.0) scale = 1 rgba = [205 105 50 255] noshadow
		GetScreenElementDims id = <id>
		if (<width> > 150)
			SetScreenElementProps id = <id> scale = 1
	...
	if ($game_mode = p2_career)
		...
		displayText \{parent = sd_container
			...
			text = "Kneifen gilt nicht! Geld und Freischaltungen richten sich nach der niedrigsten Schwierigkeitsstufe!"
			rgba = [
				225
				120
				60
