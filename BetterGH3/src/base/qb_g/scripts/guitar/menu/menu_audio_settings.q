...
script create_audio_settings_menu \{popup = 0}
	...
	if (<popup> = 1)
		...
		if NOT isSinglePlayerGame
			FormatText TextName = player_paused_text "SPIELER %d PAUSE. NUR SPIELER %d OPTIONEN SIND VERFÜGBAR." d = (($last_start_pressed_device) + 1)
			displaySprite {
	...
	if (<popup>)
		FormatText TextName = text "GITARRE: %d" d = <guitar_volume>
		<exclusive_params> = {exclusive_device = ($last_start_pressed_device)}
		displaySprite \{parent = pause_menu_frame_container
	...
	if (<popup>)
		FormatText TextName = text "SOUNDEFFEKT: %d" d = <sfx_volume>
		<exclusive_params> = {exclusive_device = ($last_start_pressed_device)}
		displaySprite \{parent = pause_menu_frame_container
			tex = HUD_score_nixie_2a
			rgba = [
