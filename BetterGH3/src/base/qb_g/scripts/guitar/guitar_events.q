...
script GuitarEvent_SongFailed_Spawned 
	...
	if ($boss_battle = 1)
		...
		FormatText TextName = winner_text "%s rockt!" s = ($current_boss.character_name)
	...
	if ($battle_sudden_death = 1)
		...
		if ($game_mode = p2_battle)
			...
			if ($is_network_game)
				...
				FormatText TextName = winner_text "Spieler %s rockt!" s = <winner>
			endif
			winner_space_between = (50.0, 0.0)
			winner_scale = 1.5
		elseif ($game_mode = p2_faceoff || $game_mode = p2_pro_faceoff)
			...
			winner_text = "Du Rockst!"
			winner_space_between = (40.0, 0.0)
			fit_dims = (350.0, 0.0)
			winner_scale = 1.0
		endif
		if ($devil_finish = 1)
			winner_text = "Mach Ihn Fertig!"
			winner_space_between = (55.0, 0.0)
			winner_scale = 1.8
		endif
		if ($current_song = bossdevil && $devil_finish = 0)
			<rock_legend> = 1
			winner_text = "DU BIST EINE"
...
script Sudden_Death_Helper_Text 
	...
	CreateScreenElement {
		...
		parent = <parent_id>
		pos = (640.0, 500.0)
		text = "Alle Powerups sind tödliche Kraftentzüge!"
		font = text_a4
		scale = 0.8
		rgba = [255 255 255 255]
		just = [center bottom]
	...
	CreateScreenElement {
		...
		parent = <parent_id>
		pos = (640.0, 540.0)
		text = "Setze einen verheerenden TÖDLICHEN KRAFTENTZUG ein!"
		font = text_a4
		scale = 0.8
		rgba = [255 255 255 255]
		just = [center bottom]
