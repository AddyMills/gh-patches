...
script add_basic_stats_desc 
	...
	add_text_to_column column = <desc_column> text = "ACIERTOS" dims = (400.0, 0.0)
	if NOT ($game_mode = p2_battle || $current_song = bosstom || $current_song = bossslash || $current_song = bossdevil)
		add_text_to_column column = <desc_column> text = "FRASES EE" dims = (400.0, 0.0)
		add_text_to_column column = <desc_column> text = "MEDIA DEL MULTIPLICADOR" dims = (400.0, 0.0)
		if ($current_num_players = 2)
			if ($game_mode = p2_faceoff ||
					$game_mode = p2_pro_faceoff ||
					$game_mode = p2_battle)
				add_text_to_column column = <desc_column> text = "TIEMPO SIENDO LÍDER" dims = (400.0, 0.0)
			endif
		endif
	else
		add_text_to_column column = <desc_column> text = "ATAQUES LANZADOS" dims = (400.0, 0.0)
	endif
endscript
