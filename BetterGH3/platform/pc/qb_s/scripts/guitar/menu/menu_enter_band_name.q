		...
		if ScreenElementExists \{id = ebn_marker}
			if (<need_unique> = 1)
				create_alert_popup \{prev_menu_id = ebn_marker
					alert = "Este nombre de grupo ya existe.  Escribe un nombre de grupo diferente."}
			else
				create_alert_popup \{prev_menu_id = ebn_marker
					alert = "¡Debes ponerle un nombre a tu grupo para continuar!"}
			endif
		endif
	else
		menu_ebn_get_band_name_text
