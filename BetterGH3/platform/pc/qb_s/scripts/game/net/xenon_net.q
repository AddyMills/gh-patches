...
script translate_server_checksums_to_strings 
	...
	if GotParam \{game_mode_checksum}
		switch (<game_mode_checksum>)
			case p2_battle
			<game_mode_value> = "GUERRA"
			case p2_faceoff
			<game_mode_value> = "DUELO"
			case p2_pro_faceoff
			<game_mode_value> = "DUELO PRO"
			if GotParam \{difficulty_checksum}
				switch (<difficulty_checksum>)
					case easy
					<game_mode_value> = "DUELO PRO: FÁCIL"
					case medium
					<game_mode_value> = "DUELO PRO: MEDIO"
					case hard
					<game_mode_value> = "DUELO PRO: DIFÍCIL"
					case expert
					<game_mode_value> = "DUELO PRO: EXP."
				endswitch
			endif
			case p2_coop
			<game_mode_value> = "MODO COOPERATIVO"
		endswitch
	endif
