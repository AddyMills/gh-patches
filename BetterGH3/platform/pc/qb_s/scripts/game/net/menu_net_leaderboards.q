...
master_leaderboard_multiplayer_list = [
	...
	}
	{
		...
		title = "DUELO: RACHA DE VICTORIAS"
		columns = 3
		headers = [
			"LUGAR"
			"NOMBRE DE USUARIO"
	...
	}
	{
		...
		title = "DUELO PRO: RACHA DE VICTORIAS"
		columns = 3
		headers = [
			"LUGAR"
			"NOMBRE DE USUARIO"
	...
	}
	{
		...
		title = "GUERRA: RACHA DE VICTORIAS"
		columns = 3
		headers = [
			"LUGAR"
			"NOMBRE DE USUARIO"
...
script create_leaderboard_groups_menu \{menu_title = "MARCADORES"
		menu_subtitle = "GRUPOS DE MARCADOR"
...
script create_leaderboard_menu 
	...
	switch ($LeaderboardSearchValue)
		case 0
		search_title = "LOS MEJORES:"
		case 1
		search_title = "MI POSICIÓN:"
	endswitch
...
script create_leaderboard_menu2 
	...
	if ($current_leaderboard_group = song)
		my_data = ["sin puntuación" "" "" "sin puntuación"]
	else
		my_data = ["sin puntuación" "" "sin puntuación"]
	endif
	if (<array_size> > 0)
...
script create_leaderboard_menu3 
	...
	add_user_control_helper \{text = "VOLVER"
		button = red
		z = 100}
	add_user_control_helper \{text = "FILTROS"
		button = Yellow
		z = 100}
	add_user_control_helper \{text = "ARRIBA/ABAJO"
		button = strumbar
...
script get_leaderboard_headers \{song = true}
	...
	if (<song> = true)
		...
		if ($checking_coop_song_flag = 1)
			...
			FormatText TextName = title "COOPERATIVA %s" s = <song_title>
		else
...
script new_leaderboard_menu 
	...
	if ((($current_leaderboard_id) = lb_faceoff_winratio) || (($current_leaderboard_id) = lb_pro_faceoff_winratio) || (($current_leaderboard_id) = lb_battle_winratio))
		if NOT ((<my_data> [2]) = "sin puntuación")
