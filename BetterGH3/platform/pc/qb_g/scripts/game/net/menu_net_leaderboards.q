...
master_leaderboard_multiplayer_list = [
	...
	}
	{
		...
		title = "GITARRENSCHLACHT - GEWINNSTRÄHNE"
		columns = 3
		headers = [
			"RANG"
			"BENUTZERNAME"
...
script create_leaderboard_groups_menu \{menu_title = "BESTENLISTEN"
		menu_subtitle = "GRUPPEN-RANGLISTEN"
...
script create_leaderboard_menu 
	...
	switch ($LeaderboardSearchValue)
		case 0
		search_title = "DIE BESTEN:"
		case 1
		search_title = "MEIN RANG:"
	endswitch
...
script create_leaderboard_menu2 
	...
	if ($current_leaderboard_group = song)
		my_data = ["kein Rang" "" "" "keine Punkte"]
	else
		my_data = ["kein Rang" "" "keine Punkte"]
	endif
	if (<array_size> > 0)
...
script create_leaderboard_menu3 
	...
	add_user_control_helper \{text = "ZURÜCK"
		button = red
		z = 100}
	add_user_control_helper \{text = "FILTER"
		button = Yellow
		z = 100}
	add_user_control_helper \{text = "OBEN/UNTEN"
		button = strumbar
...
script get_leaderboard_headers \{song = true}
	...
	if (<song> = true)
		...
		if ($checking_coop_song_flag = 1)
			...
			FormatText TextName = title "KOOP %s" s = <song_title>
		else
