...
net_game_type_info = [
	{
		name = "PRO DUELL"
		description = "Spiel einen Song mit denselben Noten."
		checksum = p2_pro_faceoff
	}
	{
		name = "DUELL"
		description = "Tritt gegen andere an und spiel Abschnitte aus Songs auf der Leadgitarre"
		checksum = p2_faceoff
	}
	{
		name = "SCHLACHT"
		description = "Battle-Modus"
		checksum = p2_battle
	}
	{
		name = "KOOP"
		description = "Kombiniere Leadgitarre und Bass-/Rhythmusgitarre."
		checksum = p2_coop
	}
]
xenon_invite_confirm_title = "SPIELEINLADUNG"
xenon_invite_confirm_dialog = "Wenn Sie diese Einladung annehmen, gehen alle Fortschritte bei Zielen verloren, an denen Sie gerade arbeiten. Wollen Sie fortfahren?"
xenon_invite_confirm_yes = "JA"
xenon_invite_confirm_no = "NEIN"
xenon_session_confirm_title = "AN SESSION TEILNEHMEN"
xenon_session_confirm_dialog = "Wenn du bei dieser Session teilnimmst, gehen alle Fortschritte bei Zielen verloren, an denen du gerade arbeitest. Möchtest du wirklich fortfahren?"
xenon_session_confirm_yes = "JA"
xenon_session_confirm_no = "NEIN"
xenon_invite_confirm_inactive_title = "SPIELEINLADUNG"
xenon_invite_confirm_inactive_dialog = "Du wurdest zu einem Online-Spiel eingeladen. Bei diesem Titel kannst du an keinem Online-Spiel teilnehmen, wenn deine Controller inaktiv sind. Daher wurde die Einladung abgelehnt."
xenon_invite_confirm_inactive_ok = "OK"
xenon_session_confirm_inactive_title = "AN SESSION TEILNEHMEN"
xenon_session_confirm_inactive_dialog = "Bei diesem Titel kannst du an keinem Online-Spiel teilnehmen, wenn deine Controller inaktiv sind. Daher kannst du bei dieser Session nicht teilnehmen."
...
script createjoinrefuseddialog 
	...
	switch (<reason>)
		...
		<connection_msg> = "DIE SESSION IST NICHT MEHR VERFÜGBAR"
		...
		<connection_msg> = "DIESER SERVER IST JETZT VOLL"
	endswitch
...
script create_connection_lost_dialog 
	...
	if (GotParam from_invite)
		FormatText \{TextName = gt_lost_connection_text
			"Der Teilnehmerserver ist voll."
			DontAssertForChecksums}
	elseif (GotParam player_quit)
		if ($opponent_gamertag = null)
			FormatText \{TextName = gt_lost_connection_text
				"Dein Gegner ist ausgestiegen"
				DontAssertForChecksums}
		else
			FormatText TextName = gt_lost_connection_text "%n\\n ist ausgestiegen" n = ($opponent_gamertag) DontAssertForChecksums
		endif
	else
		if ($opponent_gamertag = null)
			FormatText \{TextName = gt_lost_connection_text
				"VERBINDUNG ZU GEGNER VERLOREN"
				DontAssertForChecksums}
		else
			FormatText TextName = gt_lost_connection_text "VERBINDUNG ZU\\n%g UNTERBROCHEN" g = ($opponent_gamertag) DontAssertForChecksums
		endif
	endif
...
script create_failed_connection_dialog 
	...
	CreateScreenElement {
		...
		scale = 1.0
		rgba = ($online_light_blue)
		text = "VERBINDUNG MIT DEM SPIEL FEHLGESCHLAGEN"
		pos = (640.0, 315.0)
		dims = (375.0, 200.0)
		z_priority = 200.0
		just = [center center]
...
script xbox360_live_not_allowed 
	...
	if isXenon
		<sign_in_text> = "Das aktuelle Spielerprofil ist für Xbox LIVE Multiplayer-Spiele nicht aktiviert."
	else
		if GotParam \{age_restricted}
			<sign_in_text> = "Der Online-Dienst für Ihr PLAYSTATION®Network Konto ist aufgrund der Kindersicherung deaktiviert."
		else
			<sign_in_text> = "Sie müssen sich anmelden, um die Lobby betreten zu können."
		endif
	endif
	scale = 1
...
script invite_accepted 
	...
	if ((($is_network_game = 0) && ($playing_song)) &&
		...
		create_popup_warning_menu {
			textblock = {
				text = "Möchtest du diese Spielsitzung wirklich beenden?"
				pos = (640.0, 380.0)
			}
...
script create_link_unplugged_dialog 
	...
	if (GotParam reason)
		if ($winport_is_in_online_menu_system = 1)
			text = "Dein Benutzername wurde anderswo zur Anmeldung verwendet. Deine Netzwerkverbindung wurde unterbrochen. Du kehrst zum Hauptmenü zurück."
		else
			text = "Dein Benutzername wurde anderswo zur Anmeldung verwendet. Deine Netzwerkverbindung wurde unterbrochen."
		endif
	else
		if ($winport_is_in_online_menu_system = 1)
			text = "Your network connection has been disconnected. Returning to the main menu."
		else
			text = "Deine Netzwerkverbindung wurde unterbrochen."
		endif
	endif
	z = 999
	CreateScreenElement {
...
script host_wait_for_client 
	...
	create_net_popup \{popup_text = "Warten, dass der andere Spieler mitspielt"
		add_waiting_dots
		pos = (640.0, 275.0)}
	begin
...
script net_dl_content_compatabilty_warning \{z = 100
	...
	text = "Beide Spieler müssen dieselben heruntergeladenen Songs besitzen, um sie in Multiplayer-Spielen verwenden zu können."
	CreateScreenElement {
		type = TextBlockElement
		parent = <menu_anchor>
		font = text_a4
