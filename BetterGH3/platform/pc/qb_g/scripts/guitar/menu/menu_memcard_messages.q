...
script create_autologin_prompt_menu 
	memcard_cleanup_messages
	create_popup_warning_menu \{textblock = {
			text = "Möchtest du bei jedem Start von Guitar Hero III automatisch angemeldet werden? Du kannst deine Auswahl später im Menü 'Optionen' ändern."
			dims = (800.0, 400.0)
			scale = 0.5
		}
		menu_pos = (640.0, 480.0)
...
script create_signin_warning_menu 
	memcard_cleanup_messages
	create_popup_warning_menu \{textblock = {
			text = "Deine Leistung wird nur dann in den Bestenlisten aufgezeichnet, wenn du online gehst."
		}
		menu_pos = (640.0, 480.0)
...
script create_signin_complete_menu 
	memcard_cleanup_messages
	create_popup_warning_menu \{textblock = {
			text = "Dieses Spiel speichert Daten an bestimmten Stellen automatisch. Konsole nicht ausschalten, wenn die Festplatten-Kontroll-Leuchte aufleuchtet."
			pos = (640.0, 380.0)
			scale = 0.6
		}
...
script create_storagedevice_warning_menu 
	...
	if IsPs3
		...
		if ($MemcardSavingOrLoading = Saving)
			...
			desc_text = "Kein Speichermedium ausgewählt oder gefunden."
...
script create_confirm_load_menu 
	...
	switch <platform>
		case ps3
		text = "Bist du sicher, dass du diese Speicherdaten laden möchtest? Alle Fortschritte, die du seit dem letzten Speichern gemacht hast, gehen dabei verloren."
		case xenon
...
script create_no_save_found_menu 
	memcard_cleanup_messages
	create_popup_warning_menu \{textblock = {
			text = "Es wurden keine GUITAR HERO III-Speicherdaten gefunden."
		}
		menu_pos = (640.0, 480.0)
		dialog_dims = (288.0, 64.0)
		options = [
...
script create_delete_file_menu 
	...
	switch <platform>
		...
		create_popup_warning_menu \{title = "LÖSCHEN ..."
			textblock = {
				text = "Daten löschen. Schalten Sie die Konsole währenddessen nicht aus."
			}}
		case xenon
		create_popup_warning_menu \{title = "LÖSCHEN ..."
			textblock = {
...
script create_out_of_space_menu 
	...
	switch <platform>
		...
		FormatText TextName = message "Auf der Festplatte ist nicht genügend Speicherplatz vorhanden. Bei Guitar Hero 3 wird %dKB Festplattenspeicherplatz zum Speichern eines Spielstands benötigt. Wenn Sie fortfahren, können Sie Ihre Spielfortschritte nicht speichern." d = <SpaceRequired>
		create_popup_warning_menu {
			textblock = {
				text = <message>
				pos = (640.0, 390.0)
...
script create_load_file_menu 
	...
	switch <platform>
		...
		create_popup_warning_menu \{title = "LÄDT ..."
			textblock = {
				text = "Daten werden geladen. Bitte das System nicht zurücksetzen oder ausschalten."
			}}
		case xenon
		create_popup_warning_menu \{title = "LÄDT ..."
			textblock = {
...
script create_save_menu 
	...
	switch <platform>
		...
		create_popup_warning_menu \{title = "SPEICHERN ..."
			textblock = {
				text = "Auf Festplatte speichern. Bitte das System nicht zurücksetzen oder abschalten."
			}}
		case xenon
		create_popup_warning_menu \{title = "SPEICHERN ..."
			textblock = {
