		...
		if ScreenElementExists \{id = ebn_marker}
			if (<need_unique> = 1)
				create_alert_popup \{prev_menu_id = ebn_marker
					alert = "Der eingegebene Bandname existiert bereits. Bitte anderen Bandnamen eingeben."}
			else
				create_alert_popup \{prev_menu_id = ebn_marker
					alert = "Es geht weiter, wenn du einen Bandnamen eingegeben hast!"}
			endif
		endif
	else
		menu_ebn_get_band_name_text
