np_5star_headlines = [
	"%b TÖTEN IHR PUBLIKUM DURCH SCHALLANGRIFF"
	"%b SPRENGEN DIE WELT MIT IHRER UNGLAUBLICHHEIT"
	"10 VON 10 ROCKERN STEHEN AUF %b"
	"SPERRT EURE TÖCHTER EIN, %b IST IN DER STADT"
]
num_5star_headlines = 4
np_4star_headlines = [
	"%b BEEINDRUCKT PUBLIKUM MIT GUTEM SET"
	"UNSERE ÄLTEREN MITBÜRGER SIND EINER MEINUNG: %B IST ZU LAUT"
	"PUBLIKUM GENIESST GUTE SHOW VON %b"
]
num_4star_headlines = 3
np_3star_headlines = [
	"%b WAR NICHT GANZ BESCHISSEN"
	"PUBLIKUM INTERESSIERT SICH MÄSSIG FÜR %b"
	"%b KLINGT BESSER, ALS FINGERNÄGEL, DIE ÜBER EINE TAFEL KRATZEN"
]
...
script create_newspaper_menu \{for_practice = 0}
	...
	if NOT ($game_mode = p1_career || $game_mode = p2_career || $game_mode = p2_coop || $game_mode = p1_quickplay || <for_practice> = 1)
		...
		if ($game_mode = p2_battle)
			...
			FormatText TextName = winner_text "Spieler %d" d = <winner>
			if ($is_network_game)
				if (<winner> = "2")
			...
			FormatText TextName = who_won_text "%s ist der Star!" s = <winner_text>
			displayText {
				parent = newspaper_container
				text = <who_won_text>
				just = [left top]
			...
			if (<winner> = "0")
				who_won_text = "Das ist ein GLEICHSTAND!"
			else
				FormatText TextName = who_won_text "%d gewinnt um Längen!" d = <winner_text>
			endif
			CreateScreenElement {
				type = TextBlockElement
				parent = newspaper_container
		...
		if (<for_practice> = 0)
			...
			FormatText TextName = notes_hit_out_of_total "%a VON %b" a = <notes_hit> b = <notes_total>
			displayText {
				id = np_score_text
				parent = newspaper_container
...
script np_create_text \{pos = (200.0, 200.0)
		rot = 0
		text = "Kein Text zur Verfügung gestellt"
		parent = newspaper_container
		scale = 1
		rgba = [
			0
...
script np_create_options_menu \{pos = (600.0, 300.0)
	...
	if (<for_practice> = 1)
		displayText id = np_option_0 parent = newspaper_container text = "WEITER" pos = (($g_np_option_props [4].pos) + <menu_offset>) scale = (0.85, 0.7) rot = ($g_np_option_props [4].rot) font = <menu_font> noshadow
		displayText id = np_option_1 parent = newspaper_container text = "NEUSTART" pos = (($g_np_option_props [5].pos) + <menu_offset>) scale = (0.8, 0.7) rot = ($g_np_option_props [5].rot) font = <menu_font> noshadow
		displayText id = np_option_2 parent = newspaper_container text = "TEMPO ÄNDERN" pos = (($g_np_option_props [6].pos) + <menu_offset>) scale = (0.8, 0.7) rot = ($g_np_option_props [6].rot) font = <menu_font> noshadow
		displayText id = np_option_3 parent = newspaper_container text = "ABSCHNITT ÄNDERN" pos = (($g_np_option_props [7].pos) + <menu_offset>) scale = (0.8, 0.7) rot = ($g_np_option_props [7].rot) font = <menu_font> noshadow
		displayText id = np_option_4 parent = newspaper_container text = "BEENDEN" pos = (($g_np_option_props [8].pos) + <menu_offset>) scale = (0.8, 0.7) rot = ($g_np_option_props [8].rot) font = <menu_font> noshadow
		retail_menu_unfocus \{id = np_option_4}
		<initial_hl_pos> = (($g_np_option_props [4].pos) + ($g_np_option_props [4].offset) + <menu_offset>)
	else
		displayText id = np_option_0 parent = newspaper_container text = "WEITER" pos = (($g_np_option_props [0].pos) + <menu_offset>) scale = (0.85, 0.7) rot = ($g_np_option_props [0].rot) font = <menu_font> noshadow
		SetScreenElementProps id = <id> font_spacing = 2 space_spacing = 4
		if NOT ($end_credits = 1)
			if (<show_replay> = 1)
				displayText id = np_option_1 parent = newspaper_container text = "SONG WIEDERHOLEN" pos = (($g_np_option_props [1].pos) + <menu_offset>) scale = (0.8, 0.7) rot = ($g_np_option_props [1].rot) font = <menu_font> noshadow
				SetScreenElementProps id = <id> font_spacing = 2 space_spacing = 4
				displayText id = np_option_2 parent = newspaper_container text = "STATISTIKEN" pos = (($g_np_option_props [2].pos) + <menu_offset>) scale = (0.8, 0.7) rot = ($g_np_option_props [2].rot) font = <menu_font> noshadow
				SetScreenElementProps id = <id> font_spacing = 2 space_spacing = 4
			else
				displayText id = np_option_1 parent = newspaper_container text = "STATISTIKEN" pos = (($g_np_option_props [1].pos) + <menu_offset>) scale = (0.8, 0.7) rot = ($g_np_option_props [2].rot) font = <menu_font> noshadow
				SetScreenElementProps id = <id> font_spacing = 2 space_spacing = 4
			endif
		endif
