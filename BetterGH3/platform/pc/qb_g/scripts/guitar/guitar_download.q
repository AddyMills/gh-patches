...
script Downloads_EnumContent 
	...
	if IsEnumContentFilesDamaged
		...
		create_popup_warning_menu \{create_popup_warning_menu
			textblock = {
				text = "Ein heruntergeladener Inhalt ist beschädigt oder kann nicht gelesen werden. Bitte lade den Inhalt erneut herunter."
				Wait
				3
				seconds
			}
...
script create_download_scan_menu 
	...
	switch <platform>
		...
		create_popup_warning_menu \{textblock = {
				text = "Festplatte wird überprüft. Das System nicht ausschalten."
			}}
		Wait \{1
			gameframes}
		case xenon
		create_popup_warning_menu \{textblock = {
				text = "Es wird nach herunterladbarem Inhalt gesucht."
			}}
	endswitch
...
script DownloadContentLost 
	...
	printscriptinfo \{"DownloadInhaltVerloren"}
...
script DownloadContentLost_Spawned 
	...
	startrendering
	printf \{"DownloadInhaltVerloren"}
endscript

script create_downloadcontentlost_menu 
	destroy_popup_warning_menu
	create_popup_warning_menu \{title = "INHALT GEÄNDERT"
		title_props = {
			scale = 1.0
		}
		textblock = {
			text = "Es sind neue Inhalte zum Herunterladen hinzugekommen. Aus diesem Grund wurde das Spiel neu gestartet."
			pos = (640.0, 380.0)
		}
		menu_pos = (640.0, 490.0)
		dialog_dims = (288.0, 64.0)
...
script net_match_send_available_items 
	...
	if NOT IsHost
		...
		create_popup_warning_menu \{title = "ONLINE"
			...
			textblock = {
				text = "Profilinformationen werden gesendet. Bitte warten."
				pos = (640.0, 380.0)
			}}
	endif
