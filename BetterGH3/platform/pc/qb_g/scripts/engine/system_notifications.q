...
script sysnotify_handle_pause_eject 
	if IsPs3
		notify_box \{container_id = notify_eject_static_text_container
			line1 = "Die Disc wurde ausgeworfen"
			line2 = "Erneut einführen"
			menu_z = 510001}
		if (RenderingEnabled)
...
script sysnotify_handle_pause_controller 
	if IsPs3
		notify_box \{container_id = notify_controller_static_text_container
			line1 = "DU LÄSST ES ZU HEFTIG ANGEHEN"
			line2 = "Stell sicher, dass alle Controller korrekt angeschlossen sind."
			menu_z = 510000}
		if (RenderingEnabled)
			change \{pause_no_render = 0}
			fade_overlay_on
