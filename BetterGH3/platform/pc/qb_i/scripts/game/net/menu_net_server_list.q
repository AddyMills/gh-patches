...
script create_online_server_list 
	...
	if (($ui_flow_manager_state [0]) = online_server_list_fs)
		<title_text> = "SFOGLIA OCCORRENZE"
	elseif (($ui_flow_manager_state [0]) = private_match_search_fs)
		<title_text> = "PARTITA PRIVATA"
	else
		<title_text> = "PARTITA VELOCE"
	endif
	CreateScreenElement {
		type = TextElement
		parent = search_results_container
	...
	add_user_control_helper \{text = " INDIETRO"
		button = red
		z = 100}
	add_user_control_helper \{text = "AGGIORNA"
		button = Blue
		z = 100}
...
script create_server_list_searching_dialog \{menu_id = server_list_searching_dialog_menu
	...
	add_searching_menu_item vmenu_id = <vmenu_id> choose_script = searching_dialog_select_cancel text = "ANNULLA"
	set_focus_color rgba = ($online_dark_purple)
	set_unfocus_color rgba = (online_light_blue)
...
script create_server_list_create_match_dialog \{menu_id = server_list_create_match_dialog_menu
	...
	add_searching_menu_item vmenu_id = <vmenu_id> choose_script = create_match_dialog_select_create text = "CREA PARTITA"
	add_searching_menu_item vmenu_id = <vmenu_id> choose_script = create_match_dialog_select_cancel text = "ANNULLA"
