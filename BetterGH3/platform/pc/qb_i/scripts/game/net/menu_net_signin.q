...
script create_winport_connection_status_screen 
	...
	if NOT (NetSessionFunc func = IsConnected)
		add_user_control_helper \{text = "ANNULLA"
			button = red
			z = 100}
		...
		switch (<CurrentNetworkTask>)
			...
			switch (<CurrentNetworkStatus>)
				case "PENDING"
				statusText = "Inizializzazione servizi online"
				case "DONE"
				statusText = "Servizi online pronti"
				case "FAILED"
				statusText = "Impossibile inizializzare i servizi online"
				success = false
				default
				statusText = "Errore interno: stato di rete inatteso!"
				success = false
			endswitch
			case "CHECK_DNS"
			switch (<CurrentNetworkStatus>)
				case "PENDING"
				statusText = "Ricerca servizi di gioco"
				case "DONE"
				statusText = "Server di gioco trovati"
				success = true
				case "FAILED"
				statusText = "Impossibile trovare server di gioco"
				success = false
				default
				statusText = "Errore interno: stato di rete inatteso!"
				success = false
	...
	if NOT (NetSessionFunc func = HasExistingLogin)
		SetScreenElementProps \{id = statusMessage
			text = "Login esistente non trovato.\\nVuoi creare un Nuovo account o usare un Account esistente?"}
		fit_text_into_menu_item \{id = statusMessage
			max_width = 480}
		displaySprite \{parent = connectionStatusContainer
			id = options_bg_1
...
script create_winport_account_login_screen 
	...
	if (<autoLoginSetting> = autoLoginOn && NetSessionFunc func = HasExistingLogin)
		...
		create_winport_account_management_screen \{mode = loginAccount
			...
			yellowButtonText = "CAMBIA PASSWORD"
			yellowButtonAction = start_winport_account_change_screen
			blueButtonText = "NUOVO ACCOUNT"
...
script create_winport_account_change_screen 
	create_winport_account_management_screen \{mode = changeAccount
		...
		yellowButtonText = "RESETTA PASSWORD"
...
script create_winport_account_reset_screen 
	create_winport_account_management_screen \{mode = resetAccount
		title = "Azzera password"
		container = accountResetContainer
		yellowButtonText = "CANCELLA ACCOUNT"
...
script create_winport_account_management_screen 
	...
	CreateScreenElement {
		...
		text = "Il gioco potrebbe essere diverso durante la partita online"
		just = [center top]
		z_priority = 10.0
		pos = (640.0, 560.0)
		shadow
	...
	CreateScreenElement {
		...
		scale = 1.0
		rgba = [180 180 180 255]
		text = "*Utilizza la tastiera per inserire tutte le informazioni dell'account*"
		just = [center top]
		z_priority = 10.0
		pos = (640.0, 595.0)
		shadow
	...
	<pos> = (375.0, 290.0)
	create_winport_login_field container = <container> pos = <pos> label = "Nome utente: " labelId = usernameLabelId prefixId = usernamePrefixId cursorId = usernameCursorId suffixId = usernameSuffixId ang = -2.0
	...
	if (<mode> = createAccount || <mode> = changeAccount || <mode> = resetAccount)
		pos = (<pos> + ((0.0, 1.0) * <lineHeight>))
		create_winport_login_field container = <container> pos = <pos> label = "Nuova password: " labelId = newPassword1LabelId prefixId = newPassword1PrefixId cursorId = newPassword1CursorId suffixId = newPassword1SuffixId ang = -0.6
		pos = (<pos> + ((0.0, 1.0) * <lineHeight>))
		create_winport_login_field container = <container> pos = <pos> label = "Ripeti nuova password: " labelId = newPassword2LabelId prefixId = newPassword2PrefixId cursorId = newPassword2CursorId suffixId = newPassword2SuffixId ang = 0.5
	endif
	if (<mode> = createAccount || <mode> = resetAccount)
		pos = (<pos> + ((0.0, 1.0) * <lineHeight>))
		create_winport_login_field container = <container> pos = <pos> label = "Licenza: " labelId = licenseLabelId prefixId = licensePrefixId cursorId = licenseCursorId suffixId = licenseSuffixId ang = 1.5
	endif
	add_user_control_helper \{text = "ACCETTA"
		button = green
		z = 100}
	add_user_control_helper \{text = " INDIETRO"
		button = red
...
script create_winport_account_management_status_screen 
	...
	switch (<CurrentNetworkTask>)
		...
		switch (<CurrentNetworkStatus>)
			case "PENDING"
			statusText = "Richiesta creazione account"
			case "DONE"
			statusText = "Account creato"
			success = true
			case "FAILED"
			statusText = "Impossibile creare account"
			success = false
			default
			statusText = "Errore interno: stato di rete inatteso!"
			success = false
		...
		switch (<CurrentNetworkStatus>)
			case "PENDING"
			statusText = "Autorizzazione account"
			case "DONE"
			statusText = "Account autorizzato"
			success = true
			case "FAILED"
			statusText = "Impossibile autorizzare account"
			success = false
			default
			statusText = "Errore interno: stato di rete inatteso!"
			success = false
		...
		switch (<CurrentNetworkStatus>)
			case "PENDING"
			statusText = "Richiesta cambio password"
			case "DONE"
			statusText = "Password cambiata"
			success = true
			case "FAILED"
			statusText = "Impossibile cambiare la password"
			success = false
			default
			statusText = "Errore interno: stato di rete inatteso!"
			success = false
		...
		switch (<CurrentNetworkStatus>)
			case "PENDING"
			statusText = "Richiesta reset account"
			case "DONE"
			statusText = "Reset password account"
			success = true
			case "FAILED"
			statusText = "Impossibile resettare account"
			success = false
			default
			statusText = "Errore interno: stato di rete inatteso!"
			success = false
		...
		switch (<CurrentNetworkStatus>)
			case "PENDING"
			statusText = "Richiesta cancellazione account"
			case "DONE"
			statusText = "Account cancellato"
			success = true
			case "FAILED"
			statusText = "Impossibile cancellare account"
			success = false
			default
			statusText = "Errore interno: stato di rete inatteso!"
			success = false
	...
	if (<success> = false)
		...
		switch <FailureCode>
			case 666
			statusText = "I nuovi campi password non corrispondono"
			case 667
			statusText = "Servizio autorizzazione fallito"
			case 668
			statusText = "Il nome utente deve essere compreso tra 6 e 16 caratteri"
			case 669
			statusText = "La password deve essere compresa tra 6 e 16 caratteri"
			case 700
			statusText = "Operazione eseguita"
			case 701
			statusText = "Richiesta autorizzazione errata"
			case 702
			statusText = "Errore configurazione server"
			case 703
			statusText = "ID titolo gioco non valido"
			case 704
			statusText = "Informazioni account non valide"
			case 705
			statusText = "Richiesta autorizzazione non legale"
			case 706
			statusText = "Codice licenza non valido"
			case 707
			statusText = "Nome utente già esistente"
			case 708
			statusText = "Formato del nome utente non valido"
			case 709
			statusText = "Nome utente rifiutato"
			case 710
			statusText = "Troppi account per codice licenza"
			case 711
			statusText = "Migrazione account non supportata"
			case 712
			statusText = "Il titolo è stato disabilitato"
			case 713
			statusText = "L'account è scaduto"
			case 714
			statusText = "Account bloccato"
			case 715
			statusText = "Errore di autenticazione: le funzioni online saranno disponibili solo dopo aver riavviato Guitar Hero III."
			case 716
			statusText = "Password errata"
		endswitch
		SetScreenElementProps id = statusMessage text = <statusText>
		fit_text_into_menu_item \{id = statusMessage
			max_width = 480}
...
script create_join_private_menu 
	...
	CreateScreenElement \{type = TextElement
		...
		text = "Partecipa a partita privata"
		just = [
			center
			top
		]
	...
	CreateScreenElement \{type = TextElement
		...
		text = "Usa la tastiera per inserire il Nome utente della partita a cui partecipare*"
		just = [
			center
			top
		]
	...
	<pos> = (375.0, 320.0)
	create_winport_login_field container = private_menu_container pos = <pos> label = "Nome utente partita: " labelId = usernameLabelId prefixId = usernamePrefixId cursorId = usernameCursorId suffixId = usernameSuffixId ang = -2.0
	add_user_control_helper \{text = "ACCETTA"
		button = green
		z = 100}
	add_user_control_helper \{text = " INDIETRO"
...
script create_logout_submenu 
	...
	CreateScreenElement {
		...
		parent = logoutContainer
		id = statusMessage
		text = "Se selezioni Esci, la sezione online corrente sarà chiusa. I tuoi punteggi non saranno registrati in classifica fino al prossimo accesso."
		font = text_a4
		scale = 0.8
		rgba = [210 210 210 250]
		just = [center top]
...
script create_account_submenu \{menu_title = "GESTIONE ACCOUNT"
