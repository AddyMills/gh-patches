np_5star_headlines = [
	"PUBBLICO UCCISO DALL'ATTACCO SONICO DI %b"
	"%b  HA FATTO IMPAZZIRE IL MONDO CON LA SUA BELLEZZA"
	"10 ROCKER SU 10 AMANO %b"
	"CHIUDI IN CASA LE TUE FIGLIE, %b È ARRIVATO IN CITTÀ"
]
...
script create_newspaper_menu \{for_practice = 0}
	...
	if NOT ($game_mode = p1_career || $game_mode = p2_career || $game_mode = p2_coop || $game_mode = p1_quickplay || <for_practice> = 1)
		...
		if ($game_mode = p2_battle)
			...
			FormatText TextName = who_won_text "%s spacca!" s = <winner_text>
			displayText {
				parent = newspaper_container
				text = <who_won_text>
				just = [left top]
...
script np_create_options_menu \{pos = (600.0, 300.0)
	...
	if (<for_practice> = 1)
		...
		displayText id = np_option_1 parent = newspaper_container text = "RIAVVIA" pos = (($g_np_option_props [5].pos) + <menu_offset>) scale = (0.8, 0.7) rot = ($g_np_option_props [5].rot) font = <menu_font> noshadow
