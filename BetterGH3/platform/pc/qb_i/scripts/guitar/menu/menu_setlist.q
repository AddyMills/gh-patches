...
script create_sl_assets 
				...
				if (<complete> = 0)
					...
					if (<encore_unlocked> = 1)
						FormatText \{TextName = completeText
							"Supera il bis per continuare"}
					elseif (<boss_unlocked> = 1)
						FormatText \{TextName = completeText
							"Supera il brano del boss per continuare"}
					else
						GetGlobalTags <tiername> param = num_songs_to_progress
						FormatText TextName = completeText "Supera %d brani su %p per continuare" d = <num_songs_to_progress> p = <num_songs_unlocked>
					endif
	...
	if ((($game_mode = p1_career) || ($game_mode = p2_career)) && $is_demo_mode = 0)
		...
		FormatText TextName = total_score_text "Punteggio carriera: %d" d = <career_score> usecommas
		displayText {
			parent = setlist_menu
			scale = 0.7
...
script setlist_show_helperbar \{text_option1 = "BONUS"
	...
	<i> = (<i> + 1)
	repeat 5
	tabs_text = ["scaletta" "bonus" "download"]
