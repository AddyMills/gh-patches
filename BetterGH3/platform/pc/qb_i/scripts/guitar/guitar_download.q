...
script Downloads_EnumContent 
	...
	if IsEnumContentFilesDamaged
		...
		create_popup_warning_menu \{create_popup_warning_menu
			textblock = {
				text = "Un pacchetto di contenuti risulta danneggiato o illeggibile. Scarica nuovamente il pacchetto."
				Wait
				3
				seconds
			}
...
script create_download_scan_menu 
	...
	switch <platform>
		...
		create_popup_warning_menu \{textblock = {
				text = "Verifica disco rigido in corso. Non spegnere il sistema."
			}}
		Wait \{1
			gameframes}
		case xenon
		create_popup_warning_menu \{textblock = {
				text = "Ricerca contenuti da scaricare."
			}}
	endswitch
...
script DownloadContentLost 
	...
	printscriptinfo \{"ContenutoScaricatoPerduto"}
...
script DownloadContentLost_Spawned 
	...
	startrendering
	printf \{"ContenutoScaricatoPerduto"}
endscript

script create_downloadcontentlost_menu 
	destroy_popup_warning_menu
	create_popup_warning_menu \{title = "CONTENUTO CAMBIATO"
		title_props = {
			scale = 1.0
		}
		textblock = {
			text = "Download Contenuto è cambiato. Di conseguenza, il gioco è stato riavviato."
			pos = (640.0, 380.0)
		}
		menu_pos = (640.0, 490.0)
		dialog_dims = (288.0, 64.0)
...
script net_match_send_available_items 
	...
	if NOT IsHost
		...
		create_popup_warning_menu \{title = "ONLINE"
			...
			textblock = {
				text = "Invio informazioni profilo. Attendere."
				pos = (640.0, 380.0)
			}}
	endif
