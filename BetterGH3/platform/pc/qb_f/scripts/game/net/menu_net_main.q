info_text = [
	"Lance-toi dans un matchmaking d'amis et casse la baraque contre le premier rockeur venu !"
	"Lance-toi dans une partie avec classement et casse la baraque contre le premier rockeur venu !"
	"Rechercher un type de partie en ligne."
	"Rejoindre une partie privée créée par un ami."
	"Crée et héberge ta propre partie en ligne."
	"Configure et modifie les options Partie rapide."
	"Découvre qui est le meilleur et ta position dans les Classements."
	"Bientôt des téléchargements !"
	"Choisis 'Message du jour' pour consulter les dernières informations sur Guitar Hero."
	"Gérer le compte en ligne."
]
online_main_menu_pos = (470.0, 110.0)
online_info_pane_pos = (890.0, 150.0)

script create_online_main_menu \{menu_title_xenon = "Menu en ligne"
		menu_title_ps3 = "Menu en ligne"
	...
	if NOT isXenon
		if NOT NetSessionFunc \{obj = voice
			...
			CreateScreenElement {
				...
				pos = (640.0, 585.0)
				scale = (0.55, 0.55)
				text = "Le chat de votre compte PLAYSTATION®Network a été désactivé par la restriction du contrôle parental."
				font = text_a4
				rgba = ($online_light_blue)
				z_priority = 6.0
				dims = (1500.0, 120.0)
...
script create_link_text 
	...
	CreateScreenElement {
		...
		scale = (0.75, 0.75)
		rgba = ($online_light_blue)
		text = "Prêt à devenir un Guitar Hero ? Tu peux consulter tes statistiques sur le site Internet :"
		just = [center top]
		internal_just = [center top]
		z_priority = 6.0
		pos = (640.0, 160.0)
	...
	CreateScreenElement {
		...
		scale = (0.75, 0.75)
		rgba = ($online_light_blue)
		text = "- Va sur www.guitarhero.com\\n- Crée un nouveau compte ou connecte-toi\\n- Clique sur Associer compte\\n- Saisis le mot de passe VIP suivant"
		just = [center top]
		internal_just = [left top]
		z_priority = 6.0
		pos = (640.0, 240.0)
	...
	CreateScreenElement {
		...
		scale = (0.75, 0.75)
		rgba = ($online_light_blue)
		text = "Sur le Web, tu peux personnaliser ton profil, jeter un oeil sur les Classements, jouer dans un groupe en ligne, rencontrer tes groupies et participer à des tournois !"
		just = [center top]
		internal_just = [left top]
		z_priority = 6.0
		pos = (648.0, 460.0)
...
script net_add_item_to_main_menu \{line_spacing = 40}
	...
	if (<text> = "Partie rapide : partie avec classement")
