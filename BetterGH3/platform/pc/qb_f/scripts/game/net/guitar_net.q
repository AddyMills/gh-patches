...
net_game_type_info = [
	{
		name = "DUEL PRO"
		description = "Joue une chanson avec les mêmes notes."
		checksum = p2_pro_faceoff
	}
	{
		name = "DUEL"
		description = "Duel de première guitare."
		checksum = p2_faceoff
	}
	{
		name = "CHOC DES GUITARES"
		description = "mode Duel."
		checksum = p2_battle
	}
	{
		name = "COOP"
		description = "Associe première guitare et basse/guitare rythmique."
		checksum = p2_coop
	}
]
xenon_invite_confirm_title = "INVITATION À UN JEU"
xenon_invite_confirm_dialog = "Si tu acceptes l'invitation, ta progression dans la partie en cours sera perdue. Veux-tu continuer ?"
xenon_invite_confirm_yes = "OUI"
xenon_invite_confirm_no = "NON"
xenon_session_confirm_title = "REJOINDRE SESSION"
xenon_session_confirm_dialog = "Si tu rejoins cette session, ta progression dans la partie en cours sera perdue. Veux-tu continuer ?"
xenon_session_confirm_yes = "OUI"
xenon_session_confirm_no = "NON"
xenon_invite_confirm_inactive_title = "INVITATION À UN JEU"
xenon_invite_confirm_inactive_dialog = "Tu as été invité à une partie en ligne. "
xenon_invite_confirm_inactive_ok = "OK"
xenon_session_confirm_inactive_title = "REJOINDRE SESSION"
xenon_session_confirm_inactive_dialog = "Le jeu ne supporte pas les manettes inactives lors des parties en ligne, vous ne pouvez pas rejoindre la session."
...
script createjoinrefuseddialog 
	...
	switch (<reason>)
		...
		<connection_msg> = "LA SESSION N'EST PLUS DISPONIBLE"
		...
		<connection_msg> = "CE SERVEUR EST COMPLET"
	endswitch
...
script create_connection_lost_dialog 
	...
	if (GotParam from_invite)
		FormatText \{TextName = gt_lost_connection_text
			"L'invitation au serveur a été acceptée."
	...
	elseif (GotParam player_quit)
		if ($opponent_gamertag = null)
			...
			FormatText TextName = gt_lost_connection_text "%n\\n a quitté la partie" n = ($opponent_gamertag) DontAssertForChecksums
		endif
	else
		if ($opponent_gamertag = null)
			FormatText \{TextName = gt_lost_connection_text
				"CONNEXION AVEC L'ADVERSAIRE PERDUE"
				DontAssertForChecksums}
		else
			FormatText TextName = gt_lost_connection_text "CONNEXION À \\n%g PERDUE" g = ($opponent_gamertag) DontAssertForChecksums
		endif
	endif
...
script xbox360_live_not_allowed 
	...
	if isXenon
		<sign_in_text> = "Le profil du joueur ne permet pas les parties multijoueur Xbox Live."
	else
		if GotParam \{age_restricted}
			<sign_in_text> = "Le service en ligne de votre compte PLAYSTATION®Network a été désactivé par la restriction du contôle parental."
		else
			<sign_in_text> = "Tu dois être connecté pour accéder à l'accueil."
		endif
	endif
	scale = 1
...
script invite_accepted 
	...
	if ((($is_network_game = 0) && ($playing_song)) &&
		...
		create_popup_warning_menu {
			textblock = {
				text = "Veux-tu vraiment quitter cette session de jeu ?"
				pos = (640.0, 380.0)
			}
...
script create_link_unplugged_dialog 
	...
	if (GotParam reason)
		if ($winport_is_in_online_menu_system = 1)
			text = "Ton nom d'utilisateur a été employé pour se connecter ailleurs. Ta connexion réseau a été déconnectée. Retour au menu principal."
		else
			text = "Ton nom d'utilisateur a été employé pour se connecter ailleurs. Ta connexion réseau a été déconnectée."
		endif
	else
		if ($winport_is_in_online_menu_system = 1)
			text = "Your network connection has been disconnected. Returning to the main menu."
		else
			text = "Ta connexion réseau a été déconnectée."
		endif
	endif
	z = 999
	CreateScreenElement {
...
script host_wait_for_client 
	...
	create_net_popup \{popup_text = "En attente d'autres joueurs..."
		add_waiting_dots
		pos = (640.0, 275.0)}
	begin
...
script net_dl_content_compatabilty_warning \{z = 100
	...
	text = "Les deux joueurs doivent disposer des mêmes morceaux téléchargés pour les utiliser lors des parties multijoueurs."
	CreateScreenElement {
		type = TextBlockElement
		parent = <menu_anchor>
		font = text_a4
