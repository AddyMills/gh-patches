...
script create_sl_assets 
				...
				if (<complete> = 0)
					...
					if (<encore_unlocked> = 1)
						FormatText \{TextName = completeText
							"Réussis le rappel pour continuer"}
					elseif (<boss_unlocked> = 1)
						FormatText \{TextName = completeText
							"Réussis la chanson du boss pour continuer"}
					else
						GetGlobalTags <tiername> param = num_songs_to_progress
						FormatText TextName = completeText "Termine %d chanson(s) sur %p pour continuer" d = <num_songs_to_progress> p = <num_songs_unlocked>
					endif
	...
	if ((($game_mode = p1_career) || ($game_mode = p2_career)) && $is_demo_mode = 0)
		...
		FormatText TextName = total_score_text "Score Carrière : %d" d = <career_score> usecommas
		displayText {
			parent = setlist_menu
			scale = 0.7
	...
	elseif ($current_tab = tab_bonus)
		setlist_show_helperbar {
			...
			text_option1 = "SÉLECTION"
			text_option2 = "TÉLÉCHARG."
			button_option1 = "\\b6"
			button_option2 = "\\b8"
		}
	else
...
script setlist_show_helperbar \{text_option1 = "BONUS"
	...
	<i> = (<i> + 1)
	repeat 5
	tabs_text = ["sélection" "bonus" "télécharg."]
