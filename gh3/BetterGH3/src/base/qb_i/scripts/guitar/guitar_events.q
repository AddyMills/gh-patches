...
script GuitarEvent_SongFailed_Spawned 
	...
	if ($boss_battle = 1)
		...
		FormatText TextName = winner_text "Vince %s!" s = ($current_boss.character_name)
	...
	if ($battle_sudden_death = 1)
		...
		if ($game_mode = p2_battle)
			...
			if ($is_network_game)
				...
				FormatText TextName = winner_text "Vince il giocatore %s" s = <winner>
			endif
			winner_space_between = (50.0, 0.0)
			winner_scale = 1.5
		elseif ($game_mode = p2_faceoff || $game_mode = p2_pro_faceoff)
			...
			winner_text = "Sei grande!"
			winner_space_between = (40.0, 0.0)
			fit_dims = (350.0, 0.0)
			winner_scale = 1.0
		endif
		if ($devil_finish = 1)
			winner_text = "Ora finiscilo!"
...
script Sudden_Death_Helper_Text 
	...
	CreateScreenElement {
		...
		parent = <parent_id>
		pos = (640.0, 500.0)
		text = "Tutti i power-up sono attacchi mortali!"
		font = text_a4
		scale = 0.8
		rgba = [255 255 255 255]
		just = [center bottom]
	...
	CreateScreenElement {
		...
		parent = <parent_id>
		pos = (640.0, 540.0)
		text = "Lancia un devastante ATTACCO MORTALE!"
		font = text_a4
		scale = 0.8
		rgba = [255 255 255 255]
		just = [center bottom]
