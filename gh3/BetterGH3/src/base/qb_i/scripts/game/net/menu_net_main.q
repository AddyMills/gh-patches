info_text = [
	"Buttati in una partita del giocatore e sfida il primo rocker disponibile."
	"Sfida il primo rocker disponibile in una partita classificata."
	"Cerca un tipo specifico di partita online."
	"Partecipa a una partita creata da un amico."
	"Crea e ospita la tua partita online."
	"Configura e modifica le opzioni Partita veloce."
	"Guarda chi sono i migliori fra i migliori e controlla la tua classificazione."
	"Prossimamente saranno disponibili nuovi download!"
	"Seleziona 'Messaggio del giorno' per leggere le ultime notizie su Guitar Hero."
	"Gestisci account online."
]
online_main_menu_pos = (470.0, 110.0)
online_info_pane_pos = (890.0, 150.0)

script create_online_main_menu \{menu_title_xenon = "Menu principale online"
		menu_title_ps3 = "Menu principale online"
	...
	net_add_item_to_main_menu {
		VMenu = <vmenu_id>
		text = "Opzioni gioc. - Partita veloce"
		info_text_index = 5
	...
	if NOT isXenon
		if NOT NetSessionFunc \{obj = voice
			...
			CreateScreenElement {
				...
				pos = (640.0, 585.0)
				scale = (0.55, 0.55)
				text = "La chat del servizio online PLAYSTATION®Network è disabilitata sul tuo account a causa delle restrizioni attivate dal filtro contenuti."
				font = text_a4
				rgba = ($online_light_blue)
				z_priority = 6.0
				dims = (1500.0, 120.0)
...
script create_link_text 
	...
	CreateScreenElement {
		...
		scale = (0.75, 0.75)
		rgba = ($online_light_blue)
		text = "Sei pronto per essere un Guitar Hero? \\nInvia le tue statistiche alla community online:"
		just = [center top]
		internal_just = [center top]
		z_priority = 6.0
		pos = (640.0, 160.0)
	...
	CreateScreenElement {
		...
		scale = (0.75, 0.75)
		rgba = ($online_light_blue)
		text = "- Vai all'indirizzo www.guitarhero.com\\n- Crea un nuovo account o effettua l'accesso\\n- Fai clic su 'Collega account'\\n- Inserisci il seguente codice di accesso VIP"
		just = [center top]
		internal_just = [left top]
		z_priority = 6.0
		pos = (640.0, 240.0)
	...
	CreateScreenElement {
		...
		scale = (0.75, 0.75)
		rgba = ($online_light_blue)
		text = "Sul Web puoi personalizzare il tuo profilo, leggere le classifiche del gioco, suonare con un gruppo online, divertirti con le fan e partecipare a tornei!"
		just = [center top]
		internal_just = [left top]
		z_priority = 6.0
		pos = (648.0, 460.0)
...
script net_add_item_to_main_menu \{line_spacing = 40}
	...
	if (<text> = "Partita veloce: Partita classificata")
