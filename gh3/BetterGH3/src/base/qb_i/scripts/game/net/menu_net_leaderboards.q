...
master_leaderboard_multiplayer_list = [
	...
	}
	{
		...
		title = "SFIDA PRO - SERIE DI VITTORIE"
		columns = 3
		headers = [
			"CLASS."
			"NOME UTENTE"
	...
	}
	{
		...
		title = "BATTAGLIA - SERIE DI VITTORIE"
		columns = 3
		headers = [
			"CLASS."
			"NOME UTENTE"
	...
	}
	{
		...
		title = "CARRIERA COOPERATIVA - FACILE"
		columns = 3
		headers = [
			"CLASS."
			"NOME UTENTE"
	...
	}
	{
		...
		title = "CARRIERA COOPERATIVA - MEDIO"
		columns = 3
		headers = [
			"CLASS."
			"NOME UTENTE"
	...
	}
	{
		...
		title = "CARRIERA COOPERATIVA - DIFFICILE"
		columns = 3
		headers = [
			"CLASS."
			"NOME UTENTE"
	...
	}
	{
		...
		title = "CARRIERA COOPERATIVA - ESPERTO"
		columns = 3
		headers = [
			"CLASS."
			"NOME UTENTE"
...
script create_leaderboard_groups_menu \{menu_title = "CLASSIFICHE"
		menu_subtitle = "GRUPPI IN CLASSIFICA"
...
script create_leaderboard_menu 
	...
	switch ($LeaderboardSearchValue)
		case 0
		search_title = "GENERALE:"
		case 1
		search_title = "CLASSIFICAZIONE:"
	endswitch
...
script create_leaderboard_menu2 
	...
	if ($current_leaderboard_group = song)
		my_data = ["nessuna classificazione" "" "" "nessun punteggio"]
	else
		my_data = ["nessuna classificazione" "" "nessun punteggio"]
	endif
	if (<array_size> > 0)
...
script create_leaderboard_menu3 
	...
	add_user_control_helper \{text = " INDIETRO"
		button = red
		z = 100}
	add_user_control_helper \{text = "FILTRI"
		button = Yellow
		z = 100}
	add_user_control_helper \{text = "SU/GIÙ"
		button = strumbar
...
script get_leaderboard_headers \{song = true}
	...
	if (<song> = true)
		...
		if ($checking_coop_song_flag = 1)
			...
			FormatText TextName = title "%s COOPERATIVA" s = <song_title>
		else
...
script new_leaderboard_menu 
	...
	if ((($current_leaderboard_id) = lb_faceoff_winratio) || (($current_leaderboard_id) = lb_pro_faceoff_winratio) || (($current_leaderboard_id) = lb_battle_winratio))
		if NOT ((<my_data> [2]) = "nessun punteggio")
...
script block_unblock_filter_criterias 
	if NOT ($current_leaderboard_group = song)
		...
		if ScreenElementExists \{id = lb_difficulty_selection_text}
			lb_difficulty_selection_text :SetProps rgba = ($online_grey) text = "N.D."
		endif
		...
		if ScreenElementExists \{id = song_selection_text}
			song_selection_text :SetProps rgba = ($online_grey) text = "N.D."
		endif
	endif
endscript
