...
net_game_type_info = [
	{
		name = "SFIDA PRO"
		description = "Suona un brano con le stesse note."
		checksum = p2_pro_faceoff
	}
	{
		name = "SFIDA"
		description = "Sezioni di duello con la chitarra principale."
		checksum = p2_faceoff
	}
	{
		name = "BATTAGLIA"
		description = "Modalità Battaglia."
		checksum = p2_battle
	}
	{
		name = "COOPERATIVA"
		description = "Unisci la chitarra solista al basso/chitarra ritmica."
		checksum = p2_coop
	}
]
xenon_invite_confirm_title = "INVITO A GIOCARE"
xenon_invite_confirm_dialog = "Se accetti questo invito perderai i progressi relativi agli obiettivi attivi correnti. Continuare?"
xenon_invite_confirm_yes = "SÌ"
xenon_invite_confirm_no = "NO"
xenon_session_confirm_title = "ACCEDI ALLA SESSIONE"
xenon_session_confirm_dialog = "Se ti unisci a questa sessione perderai i progressi degli obiettivi attivi correnti. Continuare?"
xenon_session_confirm_yes = "SÌ"
xenon_session_confirm_no = "NO"
xenon_invite_confirm_inactive_title = "INVITO A GIOCARE"
xenon_invite_confirm_inactive_dialog = "Hai ricevuto un invito a partecipare a una partita online. Questo gioco non consente di partecipare a una partita online se il controller non è attivo, quindi l'invito è stato rifiutato."
xenon_invite_confirm_inactive_ok = "OK"
xenon_session_confirm_inactive_title = "ACCEDI ALLA SESSIONE"
xenon_session_confirm_inactive_dialog = "Questo gioco non consente di partecipare a una partita online se il controller non è attivo, quindi non è possibile accedere a questa sessione. "
...
script online_win_song 
	...
	if (<quit_early>)
		change \{player2_present = 0}
		FormatText TextName = notification_text "%n\\n ha abbandonato la partita" n = ($opponent_gamertag)
...
script createjoinrefuseddialog 
	...
	switch (<reason>)
		...
		<connection_msg> = "LA SESSIONE NON È PIÙ DISPONIBILE"
		...
		<connection_msg> = "QUESTO SERVER AL MOMENTO È PIENO"
	endswitch
...
script create_connection_lost_dialog 
	...
	if (GotParam from_invite)
		FormatText \{TextName = gt_lost_connection_text
			"Il server degli inviti è pieno."
			DontAssertForChecksums}
	elseif (GotParam player_quit)
		if ($opponent_gamertag = null)
			FormatText \{TextName = gt_lost_connection_text
				"Il tuo avversario ha abbandonato la partita"
				DontAssertForChecksums}
		else
			FormatText TextName = gt_lost_connection_text "%n\\n ha abbandonato la partita" n = ($opponent_gamertag) DontAssertForChecksums
		endif
	else
		if ($opponent_gamertag = null)
			FormatText \{TextName = gt_lost_connection_text
				"Connessione con avversario perduta"
				DontAssertForChecksums}
		else
			FormatText TextName = gt_lost_connection_text "CONNESSIONE INTERROTTA CON\\n%g" g = ($opponent_gamertag) DontAssertForChecksums
		endif
	endif
...
script create_timeout_dialog 
	...
	CreateScreenElement {
		...
		scale = 1.0
		rgba = ($online_light_blue)
		text = "TEMPO PER LA CONNESSIONE SCADUTO"
		pos = (640.0, 300.0)
		dims = (375.0, 200.0)
		z_priority = 200.0
		just = [center center]
...
script create_failed_connection_dialog 
	...
	CreateScreenElement {
		...
		scale = 1.0
		rgba = ($online_light_blue)
		text = "CONNESSIONE AL GIOCO NON RIUSCITA"
		pos = (640.0, 315.0)
		dims = (375.0, 200.0)
		z_priority = 200.0
		just = [center center]
...
script xbox360_live_not_allowed 
	...
	if isXenon
		<sign_in_text> = "Il profilo giocatore corrente non è abilitato per il gioco multiplayer su Xbox LIVE."
	else
		if GotParam \{age_restricted}
			<sign_in_text> = "Il servizio online PLAYSTATION®Network è disabilitato sul tuo account a causa delle restrizioni attivate dal filtro contenuti."
		else
			<sign_in_text> = "Devi essere connesso per entrare nella lobby."
		endif
	endif
	scale = 1
...
script invite_accepted 
	...
	if ((($is_network_game = 0) && ($playing_song)) &&
		...
		create_popup_warning_menu {
			textblock = {
				text = "Vuoi uscire da questa sessione di gioco?"
				pos = (640.0, 380.0)
			}
...
script create_link_unplugged_dialog 
	...
	if (GotParam reason)
		if ($winport_is_in_online_menu_system = 1)
			text = "Il tuo nome utente è già stato utilizzato per effettuare un accesso. La connessione sarà interrotta. Ritorno al menu principale in corso..."
		else
			text = "Il tuo nome utente è già stato utilizzato per effettuare un accesso. La connessione sarà interrotta."
		endif
	else
		if ($winport_is_in_online_menu_system = 1)
			text = "Your network connection has been disconnected. Returning to the main menu."
		else
			text = "La tua connessione di rete è stata interrotta."
		endif
	endif
	z = 999
	CreateScreenElement {
...
script host_wait_for_client 
	...
	create_net_popup \{popup_text = "In attesa che un altro giocatore si unisca"
		add_waiting_dots
		pos = (640.0, 275.0)}
	begin
...
script net_dl_content_compatabilty_warning \{z = 100
	...
	text = "Entrambi i giocatori devono avere le stesse Canzoni scaricate per usarle nelle partite multigiocatore."
	CreateScreenElement {
		type = TextBlockElement
		parent = <menu_anchor>
		font = text_a4
