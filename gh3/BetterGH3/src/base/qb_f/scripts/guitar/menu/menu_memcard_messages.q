...
script create_autologin_prompt_menu 
	memcard_cleanup_messages
	create_popup_warning_menu \{textblock = {
			text = "Veux-tu te connecter à chaque lancement de Guitar Hero III ? Tu pourras modifier ton choix par la suite dans le menu Options."
			dims = (800.0, 400.0)
			scale = 0.5
		}
		menu_pos = (640.0, 480.0)
...
script create_signin_warning_menu 
	memcard_cleanup_messages
	create_popup_warning_menu \{textblock = {
			text = "Ta performance ne sera pas suivie au classement tant que tu ne seras pas en ligne."
		}
		menu_pos = (640.0, 480.0)
...
script create_signin_complete_menu 
	memcard_cleanup_messages
	create_popup_warning_menu \{textblock = {
			text = "Cette partie sera automatiquement sauvegardée. Ne pas éteindre la console si l'indicateur d'accès disque dur clignote."
			pos = (640.0, 380.0)
			scale = 0.6
		}
...
script create_storagedevice_warning_menu 
	...
	if IsPs3
		...
		if ($MemcardSavingOrLoading = Saving)
			...
			desc_text = "Aucun périphérique de stockage sélectionné ou détecté."
...
script create_no_save_found_menu 
	memcard_cleanup_messages
	create_popup_warning_menu \{textblock = {
			text = "Données sauvegardées de GUITAR HERO III introuvables."
		}
		menu_pos = (640.0, 480.0)
		dialog_dims = (288.0, 64.0)
		options = [
...
script create_corrupted_data_menu 
	...
	switch <platform>
		...
		case xenon
		text = "Le contenu du jeu est endommagé et inutilisable. Veux-tu effacer ce contenu? Toute progression associée à ce contenu sera perdue."
	endswitch
	if ($MemcardSavingOrLoading = Saving)
		options = [
			{
...
script create_delete_file_menu 
	...
	switch <platform>
		...
		create_popup_warning_menu \{title = "SUPPRESSION EN COURS..."
			textblock = {
				text = "Suppression des données sauvegardées. Ne pas éteindre la console."
			}}
		case xenon
		create_popup_warning_menu \{title = "SUPPRESSION EN COURS..."
			textblock = {
...
script create_out_of_space_menu 
	...
	switch <platform>
		...
		FormatText TextName = message "Pas assez d'espace libre sur le disque dur pour sauvegarder les parties. Guitar Hero 3 nécessite %dKo sur le disque dur pour sauvegarder une partie. Si vous choisissez de continuer, votre progression ne sera pas sauvegardée." d = <SpaceRequired>
		create_popup_warning_menu {
			textblock = {
				text = <message>
				pos = (640.0, 390.0)
