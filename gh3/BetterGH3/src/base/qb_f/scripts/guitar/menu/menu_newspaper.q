np_5star_headlines = [
	"%b ACHÈVE SON PUBLIC AVEC SA PERFORMANCE ACOUSTIQUE."
	"%b CONQUIÈR LE MONDE ENTIER AVEC LEUR SHOW."
	"10 ROCKEURS SUR 10 SONT FANS DE %b"
	"SURVEILLE TES FILLES, %b ARRIVE EN VILLE."
]
num_5star_headlines = 4
np_4star_headlines = [
	"%b IMPRESSIONNE LE PUBLIC."
	"LES PERSONNES ÂGÉES SONT TOUTES D'ACCORD, %B JOUE TROP FORT."
	"LE PUBLIC STATISFAIT PAR LA PERFORMANCE DE %b."
]
num_4star_headlines = 3
np_3star_headlines = [
	"%b S'EN TIRE PAS TROP MAL."
	"%b NE CAPTIVE QUE MOYENNEMENT LE PUBLIC."
	"AU MOINS %b N'A PAS COLLÉ LA MIGRAINE À TOUT LE MONDE."
]
...
script create_newspaper_menu \{for_practice = 0}
	...
	if NOT ($game_mode = p1_career || $game_mode = p2_career || $game_mode = p2_coop || $game_mode = p1_quickplay || <for_practice> = 1)
		...
		if ($game_mode = p2_battle)
			...
			FormatText TextName = winner_text "Joueur %d" d = <winner>
			if ($is_network_game)
				if (<winner> = "2")
			...
			FormatText TextName = who_won_text "%s assure !" s = <winner_text>
			displayText {
				parent = newspaper_container
				text = <who_won_text>
				just = [left top]
			...
			if (<winner> = "0")
				who_won_text = "ÉGALITÉ !"
			else
				FormatText TextName = who_won_text "%d conquiert avec autorité !" d = <winner_text>
			endif
			CreateScreenElement {
				type = TextBlockElement
				parent = newspaper_container
		...
		if (<for_practice> = 0)
			...
			FormatText TextName = notes_hit_out_of_total "%a SUR %b" a = <notes_hit> b = <notes_total>
			displayText {
				id = np_score_text
				parent = newspaper_container
...
script np_create_options_menu \{pos = (600.0, 300.0)
	...
	if (<for_practice> = 1)
		displayText id = np_option_0 parent = newspaper_container text = "CONTINUER" pos = (($g_np_option_props [4].pos) + <menu_offset>) scale = (0.85, 0.7) rot = ($g_np_option_props [4].rot) font = <menu_font> noshadow
		displayText id = np_option_1 parent = newspaper_container text = "RECOMMENCER" pos = (($g_np_option_props [5].pos) + <menu_offset>) scale = (0.8, 0.7) rot = ($g_np_option_props [5].rot) font = <menu_font> noshadow
		displayText id = np_option_2 parent = newspaper_container text = "CHANGER VITESSE" pos = (($g_np_option_props [6].pos) + <menu_offset>) scale = (0.8, 0.7) rot = ($g_np_option_props [6].rot) font = <menu_font> noshadow
		displayText id = np_option_3 parent = newspaper_container text = "CHANGER SECTION" pos = (($g_np_option_props [7].pos) + <menu_offset>) scale = (0.8, 0.7) rot = ($g_np_option_props [7].rot) font = <menu_font> noshadow
		displayText id = np_option_4 parent = newspaper_container text = "QUITTER" pos = (($g_np_option_props [8].pos) + <menu_offset>) scale = (0.8, 0.7) rot = ($g_np_option_props [8].rot) font = <menu_font> noshadow
		retail_menu_unfocus \{id = np_option_4}
		<initial_hl_pos> = (($g_np_option_props [4].pos) + ($g_np_option_props [4].offset) + <menu_offset>)
	else
		displayText id = np_option_0 parent = newspaper_container text = "CONTINUER" pos = (($g_np_option_props [0].pos) + <menu_offset>) scale = (0.85, 0.7) rot = ($g_np_option_props [0].rot) font = <menu_font> noshadow
		SetScreenElementProps id = <id> font_spacing = 2 space_spacing = 4
		if NOT ($end_credits = 1)
			if (<show_replay> = 1)
				displayText id = np_option_1 parent = newspaper_container text = "RÉESSAYER" pos = (($g_np_option_props [1].pos) + <menu_offset>) scale = (0.8, 0.7) rot = ($g_np_option_props [1].rot) font = <menu_font> noshadow
				SetScreenElementProps id = <id> font_spacing = 2 space_spacing = 4
				displayText id = np_option_2 parent = newspaper_container text = "PLUS DE STATS" pos = (($g_np_option_props [2].pos) + <menu_offset>) scale = (0.8, 0.7) rot = ($g_np_option_props [2].rot) font = <menu_font> noshadow
				SetScreenElementProps id = <id> font_spacing = 2 space_spacing = 4
			else
				displayText id = np_option_1 parent = newspaper_container text = "PLUS DE STATS" pos = (($g_np_option_props [1].pos) + <menu_offset>) scale = (0.8, 0.7) rot = ($g_np_option_props [2].rot) font = <menu_font> noshadow
				SetScreenElementProps id = <id> font_spacing = 2 space_spacing = 4
			endif
		endif
