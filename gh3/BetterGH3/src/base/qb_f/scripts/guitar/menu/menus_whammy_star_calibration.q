...
script create_whammy_bar_calibration_menu \{controller = 0
	...
	<text_1> = "Appuie  complètement  sur  le  vibrato,  puis  laisse-le  doucement  revenir  à  sa  position  normale."
	button_color = "Vert"
	GetEnterButtonAssignment
	if (<assignment> = circle)
		button_color = "Rouge"
	endif
	FormatText TextName = text_2 "Appuie sur la frette %a pour calibrer sur cette position." a = <button_color>
	<text_3> = "Reproduis  le  procédé  jusqu'à  voir  le  message  \\c1''Position  normale  calibrée'' \\c0à  chaque  fois  que  le  vibrato  revient  à  sa  position  normale."
	CreateScreenElement {
	...
	CreateScreenElement {
		...
		pos = (810.0, 408.0)
		text = "POSITION NORMALE CALIBRÉE"
		just = [center top]
	...
	add_user_control_helper text = "CHOISIR" button = green z = (<z> + 100)
	add_user_control_helper text = "RETOUR" button = red z = (<z> + 100)
endscript
...
script create_star_power_trigger_calibration_menu \{controller = 0
	...
	FormatText TextName = element_text "Lève le manche de ta guitare comme si tu voulais activer le Star Power, puis appuie sur la frette %a verte pour définir cette position." a = <button_color>
	CreateScreenElement {
		type = TextBlockElement
...
script create_guitar_diagnostic_menu 
	...
	CreateScreenElement {
		<text_params>
		id = leftlength
		text = "Longueur gauche "
		pos = (580.0, 360.0)
	}
	CreateScreenElement {
		<text_params>
		id = rightlength
		text = "Longueur droite "
		pos = (580.0, 400.0)
	}
	CreateScreenElement {
		<text_params>
		id = lefttrigger
		text = "Gâchette gauche "
		pos = (580.0, 440.0)
	}
	CreateScreenElement {
		<text_params>
		id = righttrigger
		text = "Gâchette droite "
		pos = (580.0, 480.0)
	}
	CreateScreenElement {
		<text_params>
...
script update_guitar_diagnostic_menu 
	...
	if GuitarGetAnalogueInfo \{controller = 0}
		FormatText TextName = leftxtext "X gauche - %v" v = <leftx>
		FormatText TextName = rightxtext "Position du vibrato - %v" v = <rightx>
		FormatText TextName = leftytext "Y gauche - %v" v = <lefty>
		FormatText TextName = rightytext "Y droite - %v" v = <RightY>
		FormatText TextName = leftlengthtext "Longueur gauche - %v" v = <leftlength>
		FormatText TextName = rightlengthtext "Longueur droite - %v" v = <rightlength>
		FormatText TextName = lefttriggertext "Gâchette gauche - %v" v = <lefttrigger>
		FormatText TextName = righttriggertext "Gâchette droite - %v" v = <righttrigger>
		FormatText TextName = verticaldisttext "Orientation verticale - %v" v = <VerticalDist>
