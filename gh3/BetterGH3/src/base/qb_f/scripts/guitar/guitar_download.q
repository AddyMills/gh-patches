...
script Downloads_EnumContent 
	...
	if IsEnumContentFilesDamaged
		...
		create_popup_warning_menu \{create_popup_warning_menu
			textblock = {
				text = "Du contenu semble être endommagé ou illisible. Veuillez télécharger à nouveau le contenu."
				Wait
				3
				seconds
			}
...
script create_download_scan_menu 
	...
	switch <platform>
		...
		create_popup_warning_menu \{textblock = {
				text = "Vérification du disque dur en cours... Ne pas éteindre le système."
			}}
		Wait \{1
			gameframes}
		case xenon
		create_popup_warning_menu \{textblock = {
				text = "Vérification de contenu téléchargeable."
			}}
	endswitch
...
script DownloadContentLost 
	...
	printscriptinfo \{"ContenuTéléchargementPerdu"}
...
script DownloadContentLost_Spawned 
	...
	startrendering
	printf \{"ContenuTéléchargementPerdu"}
endscript

script create_downloadcontentlost_menu 
	destroy_popup_warning_menu
	create_popup_warning_menu \{title = "LE CONTENU A CHANGÉ"
		title_props = {
			scale = 1.0
		}
		textblock = {
			text = "Le contenu a été modifié. Par conséquent, le jeu a été relancé automatiquement."
			pos = (640.0, 380.0)
		}
		menu_pos = (640.0, 490.0)
		dialog_dims = (288.0, 64.0)
...
script net_match_send_available_items 
	...
	if NOT IsHost
		destroy_popup_warning_menu
		create_popup_warning_menu \{title = "EN LIGNE"
			title_props = {
				scale = 1.0
			}
			textblock = {
				text = "Envoi des informations du profil en cours. Veuillez patienter."
				pos = (640.0, 380.0)
			}}
	endif
