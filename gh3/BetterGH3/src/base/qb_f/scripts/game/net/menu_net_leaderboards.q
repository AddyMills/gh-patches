...
script create_leaderboard_groups_menu \{menu_title = "CLASSEMENTS"
		menu_subtitle = "GROUPES DE CLASSEMENT"
...
script create_leaderboard_menu 
	...
	switch ($LeaderboardSearchValue)
		case 0
		search_title = "MEILLEUR DE TOUS LES TEMPS :"
		case 1
		search_title = "MON CLASSEMENT :"
	endswitch
...
script create_leaderboard_menu2 
	...
	if ($current_leaderboard_group = song)
		my_data = ["pas de classement" "" "" "pas de score"]
	else
		my_data = ["pas de classement" "" "pas de score"]
	endif
	if (<array_size> > 0)
...
script create_leaderboard_menu3 
	...
	add_user_control_helper \{text = "RETOUR"
		button = red
		z = 100}
	add_user_control_helper \{text = "FILTRES"
		button = Yellow
		z = 100}
	add_user_control_helper \{text = "HAUT/BAS"
		button = strumbar
...
script get_leaderboard_headers \{song = true}
	...
	if (<song> = true)
		...
		if ($checking_coop_song_flag = 1)
			...
			FormatText TextName = title "COOP %s" s = <song_title>
		else
