...
script np_net_create_options_menu \{pos = (600.0, 300.0)
	...
	displayText id = np_option_0 parent = newspaper_container text = "계속" pos = (($g_np_option_props [0].pos) + <menu_offset> + (0.0, 13.0)) scale = (0.85, 0.7) rot = ($g_np_option_props [0].rot) font = <menu_font> noshadow
...
script do_tie_breaker_selection 
	...
	LaunchEvent \{type = unfocus
		target = newspaper_vmenu}
	FormatText textname = final_song_text "점수가 %d-%d로(으로) 동점입니다. 타이 브레이커 노래는 다음 플레이어에 의해 선곡됩니다..." d = (<num_songs> / 2)
