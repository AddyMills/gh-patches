...
script create_leaderboard_groups_menu \{menu_title = "순위표"
		menu_subtitle = "순위표 그룹"
...
script create_leaderboard_menu3 
	...
	add_user_control_helper \{text = "뒤로"
		button = red
		z = 100}
	add_user_control_helper \{text = "필터"
		button = Yellow
		z = 100}
	add_user_control_helper \{text = "위/아래"
		button = strumbar
...
script get_leaderboard_headers \{song = true}
	...
	if (<song> = true)
		...
		if ($checking_coop_song_flag = 1)
			...
			FormatText textname = title "협동 %s" s = <song_title>
		else
...
script block_unblock_filter_criterias 
	if NOT ($current_leaderboard_group = song)
		...
		if ScreenElementExists \{id = lb_difficulty_selection_text}
			lb_difficulty_selection_text :SetProps rgba = ($online_grey) text = "없음"
		endif
		...
		if ScreenElementExists \{id = song_selection_text}
			song_selection_text :SetProps rgba = ($online_grey) text = "없음"
		endif
	endif
endscript
