...
net_game_type_info = [
	{
		name = "프로 맞대결"
		description = "동일한 노트로 노래를 플레이 합니다."
		checksum = p2_pro_faceoff
	}
	{
		name = "맞대결"
		description = "리드 기타로 서로 대결합니다."
		checksum = p2_faceoff
	}
	{
		name = "배틀"
		description = "배틀 모드입니다."
		checksum = p2_battle
	}
	{
		name = "협동"
		description = "베이스/리듬 기타와 리드 기타를 혼합합니다."
		checksum = p2_coop
	}
]
xenon_invite_confirm_title = "게임 초대"
xenon_invite_confirm_dialog = "초대를 승인하면 현재 활성화된 목표의 진행 상황을 잃게 됩니다. 계속하시겠습니까?"
xenon_invite_confirm_yes = "YES"
xenon_invite_confirm_no = "NO"
xenon_session_confirm_title = "세션 참가"
xenon_session_confirm_dialog = "세션에 참가하면 현재 활성화된 목표의 진행 상황을 잃게 됩니다. 계속하시겠습니까?"
xenon_session_confirm_yes = "YES"
xenon_session_confirm_no = "NO"
xenon_invite_confirm_inactive_title = "게임 초대"
xenon_invite_confirm_inactive_dialog = "온라인 게임 플레이에 초대되셨습니다. 본 타이틀은 활성화되지 않은 컨트롤러로 온라인 게임에 참가하는 것을 지원하지 않으므로 초대가 거절되었습니다."
xenon_invite_confirm_inactive_ok = "확인"
xenon_session_confirm_inactive_title = "세션 참가"
xenon_session_confirm_inactive_dialog = "본 타이틀은 활성화되지 않은 컨트롤러로 온라인 게임에 참가하는 것을 지원하지 않으므로 세션에 참가할 수 없습니다."
xenon_session_confirm_inactive_ok = "확인"
NO_NET_MODE = 0
LAN_MODE = 1
INTERNET_MODE = 2
new_net_logic = 0
...
script online_win_song 
	...
	if (<quit_early>)
		change \{player2_present = 0}
		FormatText textname = notification_text "%n님이\\n 나갔음" n = ($opponent_gamertag)
...
script createjoinrefuseddialog 
	...
	switch (<reason>)
		...
		<connection_msg> = "세션을 더 이상 사용할 수 없음"
...
script create_connection_lost_dialog 
	...
	elseif (GotParam player_quit)
		if ($opponent_gamertag = null)
			...
			FormatText textname = gt_lost_connection_text "%n님이\\n 나갔음" n = ($opponent_gamertag) DontAssertForChecksums
		endif
	else
		if ($opponent_gamertag = null)
			FormatText \{textname = gt_lost_connection_text
			...
			FormatText textname = gt_lost_connection_text "%g\\n연결 끊김" g = ($opponent_gamertag) DontAssertForChecksums
		endif
	endif
...
script xbox360_live_not_allowed 
	...
	if isxenon
		<sign_in_text> = "현재 게이머 프로필이 Xbox LIVE 멀티 플레이 게임을 사용할 수 없도록 되어있습니다."
	else
		if GotParam \{age_restricted}
			<sign_in_text> = "귀하의 PLAYSTATION®Network 계정은 보호자 통제 기능으로 인해 온라인 서비스 이용이 제한되어 있습니다."
		else
			<sign_in_text> = "로비에 입장하려면 반드시 로그인하셔야 합니다."
		endif
	endif
	scale = 1
...
script create_link_unplugged_dialog 
	...
	if (GotParam reason)
		if ($winport_is_in_online_menu_system = 1)
			text = "다른 곳에서 동일한 사용자 이름으로 로그인했습니다. 네트워크 연결을 종료하고 주 메뉴로 돌아갑니다."
		else
			text = "다른 곳에서 동일한 사용자 이름으로 로그인했습니다. 네트워크 연결을 종료합니다."
		endif
	else
		if ($winport_is_in_online_menu_system = 1)
			text = "네트워크 연결이 끊겼습니다. 주 메뉴로 돌아갑니다."
		else
...
script host_wait_for_client 
	...
	create_net_popup \{popup_text = "다른 플레이어가 참가할 때까지 대기 중"
		add_waiting_dots
		pos = (640.0, 275.0)}
	begin
...
script net_dl_content_compatabilty_warning \{z = 100
	...
	text = "멀티 플레이 게임에서 다운로드한 노래를 사용하려면 두 플레이어가 모두 해당 노래를 가지고 있어야 합니다."
	CreateScreenElement {
		type = TextBlockElement
		parent = <menu_anchor>
		font = text_a4
