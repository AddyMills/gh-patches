np_5star_headlines = [
	"%b님의 음파 공격으로 관중들이 쓰러지고 있습니다"
	"%b이(가) 자신들의 위대함으로 전세계를 달구고 있습니다"
	"10명의 락커 중 전원이 %b님을 좋아합니다"
	"따님들을 모두 가둬두십시오. %b님이 마을에 찾아왔습니다"
]
num_5star_headlines = 4
np_4star_headlines = [
	"%b님이 훌륭한 세트로 관중에게 좋은 인상을 심어주고 있습니다"
	"중년의 시민들은 %B님이 너무 시끄럽다고 느끼고 있습니다"
	"관중들이 %b님의 훌륭한 쇼를 즐기고 있습니다"
]
num_4star_headlines = 3
np_3star_headlines = [
	"%b님이 연주를 너무 못하고 있습니다"
	"관중이 조용히 %b에 관심을 보이고 있습니다"
	"%b님의 연주는 칠판을 손톱으로 긁는 소리보다 약간 나은 정도입니다"
]
...
script create_newspaper_menu \{for_practice = 0}
	...
	if NOT ($game_mode = p1_career || $game_mode = p2_career || $game_mode = p2_coop || $game_mode = p1_quickplay || <for_practice> = 1)
		...
		if ($game_mode = p2_battle)
			...
			FormatText textname = winner_text "플레이어 %d" d = <winner>
			if ($is_network_game)
				if (<winner> = "2")
			...
			if (<winner> = "0")
				...
				FormatText textname = who_won_text "%s님이 압도하고 있습니다!" d = <winner_text>
			endif
			CreateScreenElement {
				type = TextBlockElement
				parent = newspaper_container
		...
		if (<for_practice> = 0)
			...
			FormatText textname = notes_hit_out_of_total "%b 중 %a" a = <notes_hit> b = <notes_total>
			displayText {
				id = np_score_text
				parent = newspaper_container
...
script np_create_options_menu \{pos = (600.0, 300.0)
	...
	if (<for_practice> = 1)
		displayText id = np_option_0 parent = newspaper_container text = "계속" pos = (($g_np_option_props [4].pos) + <menu_offset>) scale = (0.85, 0.7) rot = ($g_np_option_props [4].rot) font = <menu_font> noshadow
		displayText id = np_option_1 parent = newspaper_container text = "다시 시작" pos = (($g_np_option_props [5].pos) + <menu_offset>) scale = (0.8, 0.7) rot = ($g_np_option_props [5].rot) font = <menu_font> noshadow
		displayText id = np_option_2 parent = newspaper_container text = "속도 변경" pos = (($g_np_option_props [6].pos) + <menu_offset>) scale = (0.8, 0.7) rot = ($g_np_option_props [6].rot) font = <menu_font> noshadow
		displayText id = np_option_3 parent = newspaper_container text = "부분 변경" pos = (($g_np_option_props [7].pos) + <menu_offset>) scale = (0.8, 0.7) rot = ($g_np_option_props [7].rot) font = <menu_font> noshadow
		displayText id = np_option_4 parent = newspaper_container text = "종료" pos = (($g_np_option_props [8].pos) + <menu_offset>) scale = (0.8, 0.7) rot = ($g_np_option_props [8].rot) font = <menu_font> noshadow
		retail_menu_unfocus \{id = np_option_4}
		<initial_hl_pos> = (($g_np_option_props [4].pos) + ($g_np_option_props [4].offset) + <menu_offset>)
	else
		displayText id = np_option_0 parent = newspaper_container text = "계속" pos = (($g_np_option_props [0].pos) + <menu_offset>) scale = (0.85, 0.7) rot = ($g_np_option_props [0].rot) font = <menu_font> noshadow
		SetScreenElementProps id = <id> font_spacing = 2 space_spacing = 4
		if NOT ($end_credits = 1)
			if (<show_replay> = 1)
				displayText id = np_option_1 parent = newspaper_container text = "다시 시도" pos = (($g_np_option_props [1].pos) + <menu_offset>) scale = (0.8, 0.7) rot = ($g_np_option_props [1].rot) font = <menu_font> noshadow
				SetScreenElementProps id = <id> font_spacing = 2 space_spacing = 4
				displayText id = np_option_2 parent = newspaper_container text = "다른 통계" pos = (($g_np_option_props [2].pos) + <menu_offset>) scale = (0.8, 0.7) rot = ($g_np_option_props [2].rot) font = <menu_font> noshadow
				SetScreenElementProps id = <id> font_spacing = 2 space_spacing = 4
			else
				displayText id = np_option_1 parent = newspaper_container text = "다른 통계" pos = (($g_np_option_props [1].pos) + <menu_offset>) scale = (0.8, 0.7) rot = ($g_np_option_props [2].rot) font = <menu_font> noshadow
				SetScreenElementProps id = <id> font_spacing = 2 space_spacing = 4
			endif
		endif
