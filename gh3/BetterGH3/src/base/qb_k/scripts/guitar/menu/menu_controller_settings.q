...
script winport_create_p1_controller_popup 
	create_popup_warning_menu \{title = "컨트롤러 선택"
		textblock = {
			text = "싱글 플레이 모드와 메뉴에서 사용할 컨트롤러의 버튼을 누르십시오."
			dims = (800.0, 400.0)
			scale = 0.55
		}
		no_background
...
script create_controller_settings_menu \{popup = 0}
	...
	if (<popup>)
		<p1_l_flip_text> = "P1 왼손잡이: 사용 안 함"
		<p2_l_flip_text> = "P2 왼손잡이: 사용 안 함"
	else
		<p1_l_flip_text> = "P1 왼손잡이:"
		<p2_l_flip_text> = "P2 왼손잡이:"
	endif
	CreateScreenElement {
		<text_params>
		text = <p1_l_flip_text>
