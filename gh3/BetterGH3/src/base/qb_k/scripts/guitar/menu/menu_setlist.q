...
script create_sl_assets 
			...
			if ((($game_mode = p1_career) || ($game_mode = p2_career)) && (GotParam tab_setlist) && $is_demo_mode = 0)
				...
				if (<complete> = 0)
					...
					elseif (<boss_unlocked> = 1)
						...
						FormatText textname = completetext "계속하려면 %p개의 노래 중 %d개의 노래를 완료" d = <num_songs_to_progress> p = <num_songs_unlocked>
					endif
	...
	if ((($game_mode = p1_career) || ($game_mode = p2_career)) && $is_demo_mode = 0)
		...
		FormatText textname = total_score_text "경력 점수: %d" d = <career_score> usecommas
		displayText {
			parent = setlist_menu
			scale = 0.7
...
script setlist_show_helperbar \{text_option1 = "보너스"
		text_option2 = "다운로드"
		button_option1 = "\\b7"
		button_option2 = "\\b8"
		spacing = 16}
