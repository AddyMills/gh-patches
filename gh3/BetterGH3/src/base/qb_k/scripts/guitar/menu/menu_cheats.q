...
script create_cheats_menu 
	...
	if ($entering_cheat = 0)
		...
		switch <platform>
			case xenon
			warning = "경고: 활성화된 치트 중 몇몇은 경력 모드와 온라인에서는 동작하지 않습니다."
			warning_cont = "또한 도전 과제 잠금 해제와 순위표 갱신 기능도 사용이 중지됩니다."
			case ps3
			warning = "경고: 활성화된 치트 중 몇몇은 경력 모드와 온라인에서는 동작하지 않습니다."
			warning_cont = "또한 순위표 갱신 기능도 사용이 중지됩니다."
			case ps2
			warning = "경고: 활성화된 치트 중 몇몇은 경력 모드에서는 동작하지 않습니다."
			warning_cont = ""
			default
			warning = "경고: 활성화된 치트 중 몇몇은 경력 모드와 온라인에서는 동작하지 않습니다."
			warning_cont = "또한 순위표 갱신 기능도 사용이 중지됩니다."
	...
	if (<unlock_cheat_nofail> > 0)
		if ($cheat_nofail = 1)
			FormatText textname = text "%c : 사용" c = ($guitar_hero_cheats [3].name_text)
		else
			if ($cheat_nofail < 0)
				change \{cheat_nofail = 2}
			endif
			FormatText textname = text "%c : 사용 안 함" c = ($guitar_hero_cheats [3].name_text)
		endif
	endif
	CreateScreenElement {
		<text_params2>
	...
	if (<unlock_cheat_airguitar> > 0)
		if ($cheat_airguitar = 1)
			FormatText textname = text "%c : 사용" c = ($guitar_hero_cheats [0].name_text)
		else
			if ($cheat_airguitar < 0)
				change \{cheat_airguitar = 2}
			endif
			FormatText textname = text "%c : 사용 안 함" c = ($guitar_hero_cheats [0].name_text)
		endif
	endif
	CreateScreenElement {
		<text_params2>
	...
	if (<unlock_cheat_hyperspeed> > 0)
		if ($Cheat_HyperSpeed > 0)
			FormatText textname = text "%c : 사용" c = ($guitar_hero_cheats [2].name_text)
			FormatText textname = text "%c, %d" c = <text> d = ($Cheat_HyperSpeed)
		else
			if ($Cheat_HyperSpeed < 0)
				change \{Cheat_HyperSpeed = 0}
			endif
			FormatText textname = text "%c : 사용 안 함" c = ($guitar_hero_cheats [2].name_text)
		endif
	endif
	CreateScreenElement {
		<text_params2>
	...
	if (<unlock_cheat_performancemode> > 0)
		if ($Cheat_PerformanceMode = 1)
			FormatText textname = text "%c : 사용" c = ($guitar_hero_cheats [1].name_text)
		else
			if ($Cheat_PerformanceMode < 0)
				change \{Cheat_PerformanceMode = 2}
			endif
			FormatText textname = text "%c : 사용 안 함" c = ($guitar_hero_cheats [1].name_text)
		endif
	endif
	CreateScreenElement {
		<text_params2>
	...
	if (<unlock_cheat_easyexpert> > 0)
		if ($cheat_easyexpert = 1)
			FormatText textname = text "%c : 사용" c = ($guitar_hero_cheats [4].name_text)
		else
			if ($cheat_easyexpert < 0)
				change \{cheat_easyexpert = 2}
			endif
			FormatText textname = text "%c : 사용 안 함" c = ($guitar_hero_cheats [4].name_text)
		endif
	endif
	CreateScreenElement {
		<text_params2>
	...
	if (<unlock_cheat_precisionmode> > 0)
		if ($cheat_precisionmode = 1)
			FormatText textname = text "%c : 사용" c = ($guitar_hero_cheats [5].name_text)
		else
			if ($cheat_precisionmode < 0)
				change \{cheat_precisionmode = 2}
			endif
			FormatText textname = text "%c : 사용 안 함" c = ($guitar_hero_cheats [5].name_text)
		endif
	endif
	CreateScreenElement {
		<text_params2>
	...
	if (<unlock_cheat_bretmichaels> > 0)
		if ($cheat_bretmichaels = 1)
			FormatText textname = text "%c : 사용" c = ($guitar_hero_cheats [6].name_text)
		else
			if ($cheat_bretmichaels < 0)
				change \{cheat_bretmichaels = 2}
			endif
			FormatText textname = text "%c : 사용 안 함" c = ($guitar_hero_cheats [6].name_text)
		endif
	endif
	CreateScreenElement {
		<text_params2>
...
script cheats_zoomin_guitar 
	...
	add_user_control_helper \{text = "치트 입력 종료"
		button = start
		z = 100
		fit_to_rectangle = 0}
	add_user_control_helper \{text = "위/아래로 스트럼"
		button = strumbar
		z = 100
		fit_to_rectangle = 0}
...
script toggle_hyperspeed 
	if ($<cheat> >= 0)
		if ($<cheat> = 5)
			...
			FormatText textname = text "%c : 사용 안 함" c = ($guitar_hero_cheats [<index>].name_text)
			SetScreenElementProps id = <id> text = <text>
		else
			change GlobalName = <cheat> newvalue = ($<cheat> + 1)
			FormatText textname = text "%c : 사용" c = ($guitar_hero_cheats [<index>].name_text)
			FormatText textname = text "%c, %d" c = <text> d = ($Cheat_HyperSpeed)
			SetScreenElementProps id = <id> text = <text>
		endif
	endif
...
script toggle_cheat 
	if ($<cheat> > 0)
		...
		SetScreenElementProps id = <id> text = "잠김"
	endif
	show_cheat_warning
endscript
