...
script winport_create_calibrate_lag_menu \{from_in_game = 1}
	...
	upper_helper = "이 값은 0ms에서 200ms 사이의 값으로 설정할 수 있습니다.\\n\\n몇몇 컴퓨터 하드웨어에서는 게임 플레이와 음향이 서로 맞지 않을 수도 있습니다. 노트가 음악보다 더 빠른 것 같다면 20ms를 더해보시고 음악보다 느리다면 20ms를 빼보십시오.\\n그리고 그 값을 기본으로 취향에 맞도록 조절하십시오."
	CreateScreenElement {
...
script winport_menu_calibrate_lag_create_circles 
	...
	add_user_control_helper \{text = "스트럼"
		button = strumbar
		z = 100}
	destroy_menu \{menu_id = winport_cl_scroll}
...
script winport_menu_calibrate_lag_say_lines_are_even 
	...
	output_log_text "조정: %o" o = <new_input_diff> color = white
...
script winport_create_audio_calibrate_reminder 
	...
	CreateScreenElement {
		...
		pos = (640.0, 420.0)
		dims = (650.0, 500.0)
		text = "특정 컴퓨터 하드웨어에서 게임 플레이를 최적화하려면 옵션 메뉴에 있는 음향 랙 조절 도구를 사용하십시오."
		scale = 0.7
	}
