...
script Downloads_EnumContent 
	...
	if IsEnumContentFilesDamaged
		...
		create_popup_warning_menu \{create_popup_warning_menu
			textblock = {
				text = "콘텐츠 패키지가 손상되었거나 읽을 수 없습니다. 콘텐츠 패키지를 다시 다운로드하십시오."
				Wait
				3
				seconds
			}
...
script create_downloadcontentlost_menu 
	destroy_popup_warning_menu
	create_popup_warning_menu \{title = "콘텐츠 변경됨"
		title_props = {
			scale = 1.0
		}
		textblock = {
			text = "다운로드 콘텐츠가 변경되었습니다. 그러므로 게임을 다시 시작해야합니다."
			pos = (640.0, 380.0)
		}
		menu_pos = (640.0, 490.0)
		dialog_dims = (288.0, 64.0)
...
script net_match_send_available_items 
	...
	if NOT IsHost
		destroy_popup_warning_menu
		create_popup_warning_menu \{title = "온라인"
			title_props = {
				scale = 1.0
			}
			textblock = {
				text = "프로필 정보를 전송 중입니다. 잠시 기다려주십시오."
				pos = (640.0, 380.0)
			}}
	endif
