...
script winport_create_p1_controller_popup 
	create_popup_warning_menu \{title = "Controller wählen"
		textblock = {
			text = "Drücke die Controllertaste, die im Einzelspielermodus und in den Menüs verwendet werden soll."
			dims = (800.0, 400.0)
			scale = 0.55
		}
		no_background
	...
	add_user_control_helper \{text = "ZURÜCK"
		button = red
		z = 100}
	spawnscriptnow \{winport_wait_for_device_press
		params = {
			backScript = winport_p1_controller_back
			proceedScript = winport_p1_controller_proceed
		}}
endscript
...
script winport_create_bind_buttons_popup 
	create_popup_warning_menu \{title = "Controller wählen"
		textblock = {
			text = "Drücke die Controllertaste, die du konfigurieren möchtest."
			dims = (800.0, 400.0)
			scale = 0.55
		}
		no_background
	...
	spawnscriptnow \{winport_wait_for_device_press
		params = {
			bindable
			backScript = winport_bind_buttons_back
			proceedScript = winport_bind_buttons_proceed
		}}
endscript
...
script create_controller_settings_menu \{popup = 0}
	...
	if (<popup>)
		<p1_l_flip_text> = "S1 Linkshändereinstellung: AUS"
		<p2_l_flip_text> = "S2 Linkshändereinstellung: AUS"
	else
		<p1_l_flip_text> = "S1 Linkshändereinstellung:"
		<p2_l_flip_text> = "S2 Linkshändereinst.:"
	endif
	CreateScreenElement {
		<text_params>
		text = <p1_l_flip_text>
...
script controller_settings_menu_update_lefty_flip_p1_value \{lefty_flip_p1 = 0}
	if (<lefty_flip_p1>)
		...
		if NOT ($cs_is_popup)
			...
			SetScreenElementProps \{id = lefty_flip_p1_se
				text = "S1 Linkshändereinstellung: AUS"}
		endif
	endif
endscript
	...
	if (<lefty_flip_p2>)
		if NOT ($cs_is_popup)
			...
			SetScreenElementProps \{id = lefty_flip_p2_se
				text = "S2 Linkshändereinstellung: EIN"}
		endif
	else
		if NOT ($cs_is_popup)
			SetScreenElementProps \{id = cs_check_2
			...
			SetScreenElementProps \{id = lefty_flip_p2_se
				text = "S2 Linkshändereinstellung: AUS"}
		endif
	endif
endscript
...
script controller_settings_show_default_keyboard_layout 
	ui_flow_manager_respond_to_action \{action = show_default_keyboard_layout
		create_params = {
			keyboard = 1
		}}
endscript
