...
script create_sl_assets 
				...
				if (<complete> = 0)
					...
					if (<encore_unlocked> = 1)
						FormatText \{TextName = completeText
							"Meistere den Zugabe-Song, um fortzufahren"}
					elseif (<boss_unlocked> = 1)
						FormatText \{TextName = completeText
							"Meistere den Boss-Song, um fortzufahren"}
					else
						GetGlobalTags <tiername> param = num_songs_to_progress
						FormatText TextName = completeText "Meistere %d von %p Songs, um fortzufahren" d = <num_songs_to_progress> p = <num_songs_unlocked>
					endif
	...
	if ((($game_mode = p1_career) || ($game_mode = p2_career)) && $is_demo_mode = 0)
		...
		FormatText TextName = total_score_text "Karrierepunkte: %d" d = <career_score> usecommas
		displayText {
			parent = setlist_menu
			scale = 0.7
...
script setlist_show_helperbar \{text_option1 = "BONUSSONGS"
		text_option2 = "DOWNLOADS"
		button_option1 = "\\b7"
		button_option2 = "\\b8"
		spacing = 16}
	...
	<i> = (<i> + 1)
	repeat 5
	tabs_text = ["Setliste" "Bonussongs" "Downloads"]
