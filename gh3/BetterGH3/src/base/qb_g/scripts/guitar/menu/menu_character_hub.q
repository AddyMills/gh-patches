...
script create_character_hub_menu \{player = 1}
	...
	if ($current_num_players = 2)
		...
		if (($is_network_game) = 1)
			...
			<continue_id> :SetProps text = "Bereit" rgba = [128 128 128 255] event_handlers = <net_event_handlers> replace_handlers block_events
...
script create_select_guitar_finish_menu \{player = 1}
	...
	if (<player> = 1)
		if ($current_num_players = 2)
			new_menu scrollid = scrolling_select_finish_p1 vmenuid = vmenu_select_finish_p1 name = "Gitarrenlackierung wählen" use_backdrop = 0 exclusive_device = <exclusive_dev> event_handlers = <event_handlers>
		else
			new_menu scrollid = scrolling_select_finish_p1 vmenuid = vmenu_select_finish_p1 name = "Gitarrenlackierung wählen" use_backdrop = 0 event_handlers = <event_handlers>
		endif
		vparent = vmenu_select_finish_p1
	else
		new_menu scrollid = scrolling_select_finish_p2 vmenuid = vmenu_select_finish_p2 name = "Gitarrenlackierung wählen" use_backdrop = 0 exclusive_device = <exclusive_dev> event_handlers = <event_handlers>
