...
script create_whammy_bar_calibration_menu \{controller = 0
	...
	<text_1> = "Bewege das Tremolo bei langen Noten komplett nach unten und lass es in die Ausgangsposition zurückkehren."
	button_color = "Grün"
	GetEnterButtonAssignment
	if (<assignment> = circle)
		button_color = "Rot"
	endif
	FormatText TextName = text_2 "Drück die %a Taste, um das Tremolo in der aktuellen Position zu kalibrieren." a = <button_color>
	<text_3> = "Wiederhole den Ablauf jedes Mal, wenn du das Tremolo in seine Ruhestellung bringst, bis du die Meldung \\c1 'Ruhestellung kalibriert' \\c0siehst."
	CreateScreenElement {
	...
	add_user_control_helper text = "WÄHLEN" button = green z = (<z> + 100)
	add_user_control_helper text = "ZURÜCK" button = red z = (<z> + 100)
endscript
...
script create_star_power_trigger_calibration_menu \{controller = 0
	...
	FormatText TextName = element_text "Zieh deine Gitarre bis zu dem Punkt hoch, an dem die Starpower aktiviert werden soll. Drück dann die %a grüne Grifftaste, um diesen Wert festzulegen." a = <button_color>
	CreateScreenElement {
		type = TextBlockElement
...
script create_guitar_diagnostic_menu 
	...
	CreateScreenElement {
		<text_params>
		id = lefttrigger
		text = "Linker Schalter "
		pos = (580.0, 440.0)
	}
	CreateScreenElement {
		<text_params>
		id = righttrigger
		text = "Rechter Schalter "
		pos = (580.0, 480.0)
	}
	CreateScreenElement {
		<text_params>
		id = VerticalDist
		text = "Vertikale Distanz "
		pos = (580.0, 520.0)
	}
...
script update_guitar_diagnostic_menu 
	...
	if GuitarGetAnalogueInfo \{controller = 0}
		FormatText TextName = leftxtext "Links X - %v" v = <leftx>
		FormatText TextName = rightxtext "Tremolo-Position - %v" v = <rightx>
		FormatText TextName = leftytext "Links Y - %v" v = <lefty>
		FormatText TextName = rightytext "Right Y - %v" v = <RightY>
		FormatText TextName = leftlengthtext "Linke Länge - %v" v = <leftlength>
		FormatText TextName = rightlengthtext "Rechte Länge - %v" v = <rightlength>
		FormatText TextName = lefttriggertext "Linker Schalter - %v" v = <lefttrigger>
		FormatText TextName = righttriggertext "Rechter Schalter - %v" v = <righttrigger>
		FormatText TextName = verticaldisttext "Vertikale Ausrichtung - %v" v = <VerticalDist>
