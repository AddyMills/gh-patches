...
gs_instrument_blurb_text = "INFORMATIONEN ZUM STANDARD-INSTRUMENT"
gs_select_text = "GITARRE WÄHLEN"
gs_element_offsets = [
	(0.0, 125.0)
	(-4.0, 185.0)
	(-4.0, 200.0)
...
script create_select_guitar_menu \{player = 1
	...
	if ($current_num_players = 1)
		...
		if (<instrument_type> = guitar)
			change \{gs_select_text = "GITARRE WÄHLEN"}
		elseif (<instrument_type> = Bass)
			change \{gs_select_text = "BASS WÄHLEN"}
		endif
		if (<player> = 1)
			FormatText checksumname = player_status 'player%i_status' i = <player>
			if find_profile_by_id id = ($<player_status>.character_id)
...
script get_player_instrument_desc_name \{player = 1}
	...
	if find_instrument_index desc_id = (<instrument_id>)
		...
		return \{false
			instrument_name = "Instrument nicht gefunden"}
	endif
endscript
