guitar_hero_cheats = [
	...
	{
		name = hyperspeed
		name_text = "Hypergeschwindigkeit"
	...
	{
		name = NoFail
		name_text = "kein Song misslungen"
		var = Cheat_NoFail
		unlock_pattern = [
			69632
			16
	...
	{
		name = unlockall
		name_text = "alle Songs freigeschaltet"
		var = Cheat_UnlockAll
		unlock_pattern = [
			257
			4112
	...
	}
	{
		...
		name_text = "alles freigeschaltet"
...
script create_cheats_menu 
	...
	if ($entering_cheat = 0)
		...
		switch <platform>
			case xenon
			warning = "WARNUNG: Manche aktiven Cheats funktionieren nicht, wenn du dich im Karriere-Modus befindest oder online bist."
			warning_cont = "Außerdem sind das Freischalten von Erfolgen und das Ansehen der Bestenlisten-Plätze nicht möglich."
			case ps3
			warning = "WARNUNG: Manche aktiven Cheats funktionieren nicht, wenn du dich im Karriere-Modus befindest oder online bist."
			warning_cont = "Außerdem ist das Ansehen der Bestenlisten-Plätze nicht möglich."
			case Ps2
			warning = "WARNUNG: Manche aktiven Cheats funktionieren nicht, wenn du dich im Karriere-Modus befindest."
			warning_cont = ""
			default
			warning = "WARNUNG: Manche aktiven Cheats funktionieren nicht, wenn du dich im Karriere-Modus befindest oder online bist."
			warning_cont = "Außerdem ist das Ansehen der Bestenlisten-Plätze nicht möglich."
	...
	if (<unlock_Cheat_NoFail> > 0)
		if ($Cheat_NoFail = 1)
			FormatText TextName = text "%c : EIN" c = ($guitar_hero_cheats [3].name_text)
		else
			if ($Cheat_NoFail < 0)
				change \{Cheat_NoFail = 2}
			endif
			FormatText TextName = text "%c : AUS" c = ($guitar_hero_cheats [3].name_text)
		endif
	endif
	CreateScreenElement {
		<text_params2>
	...
	if (<unlock_Cheat_AirGuitar> > 0)
		if ($Cheat_AirGuitar = 1)
			FormatText TextName = text "%c : EIN" c = ($guitar_hero_cheats [0].name_text)
		else
			if ($Cheat_AirGuitar < 0)
				change \{Cheat_AirGuitar = 2}
			endif
			FormatText TextName = text "%c : AUS" c = ($guitar_hero_cheats [0].name_text)
		endif
	endif
	CreateScreenElement {
		<text_params2>
	...
	if (<unlock_Cheat_Hyperspeed> > 0)
		if ($Cheat_Hyperspeed > 0)
			FormatText TextName = text "%c : EIN" c = ($guitar_hero_cheats [2].name_text)
			FormatText TextName = text "%c, %d" c = <text> d = ($Cheat_Hyperspeed)
		else
			if ($Cheat_Hyperspeed < 0)
				change \{Cheat_Hyperspeed = 0}
			endif
			FormatText TextName = text "%c : AUS" c = ($guitar_hero_cheats [2].name_text)
		endif
	endif
	CreateScreenElement {
		<text_params2>
	...
	if (<unlock_Cheat_PerformanceMode> > 0)
		if ($Cheat_PerformanceMode = 1)
			FormatText TextName = text "%c : EIN" c = ($guitar_hero_cheats [1].name_text)
		else
			if ($Cheat_PerformanceMode < 0)
				change \{Cheat_PerformanceMode = 2}
			endif
			FormatText TextName = text "%c : AUS" c = ($guitar_hero_cheats [1].name_text)
		endif
	endif
	CreateScreenElement {
		<text_params2>
	...
	if (<unlock_Cheat_EasyExpert> > 0)
		if ($Cheat_EasyExpert = 1)
			FormatText TextName = text "%c : EIN" c = ($guitar_hero_cheats [4].name_text)
		else
			if ($Cheat_EasyExpert < 0)
				change \{Cheat_EasyExpert = 2}
			endif
			FormatText TextName = text "%c : AUS" c = ($guitar_hero_cheats [4].name_text)
		endif
	endif
	CreateScreenElement {
		<text_params2>
	...
	if (<unlock_Cheat_PrecisionMode> > 0)
		if ($Cheat_PrecisionMode = 1)
			FormatText TextName = text "%c : EIN" c = ($guitar_hero_cheats [5].name_text)
		else
			if ($Cheat_PrecisionMode < 0)
				change \{Cheat_PrecisionMode = 2}
			endif
			FormatText TextName = text "%c : AUS" c = ($guitar_hero_cheats [5].name_text)
		endif
	endif
	CreateScreenElement {
		<text_params2>
	...
	if (<unlock_Cheat_BretMichaels> > 0)
		if ($Cheat_BretMichaels = 1)
			FormatText TextName = text "%c : EIN" c = ($guitar_hero_cheats [6].name_text)
		else
			if ($Cheat_BretMichaels < 0)
				change \{Cheat_BretMichaels = 2}
			endif
			FormatText TextName = text "%c : AUS" c = ($guitar_hero_cheats [6].name_text)
		endif
	endif
	CreateScreenElement {
		<text_params2>
...
script cheats_zoomin_guitar 
	...
	add_user_control_helper \{text = "CHEAT-EINGABE BEENDEN"
		button = start
		z = 100
		fit_to_rectangle = 0}
	add_user_control_helper \{text = "NACH OBEN/UNTEN ANSCHLAGEN"
		button = strumbar
		z = 100
		fit_to_rectangle = 0}
...
script toggle_hyperspeed 
	if ($<cheat> >= 0)
		if ($<cheat> = 5)
			...
			FormatText TextName = text "%c : AUS" c = ($guitar_hero_cheats [<index>].name_text)
			SetScreenElementProps id = <id> text = <text>
		else
			change globalname = <cheat> newvalue = ($<cheat> + 1)
			FormatText TextName = text "%c : EIN" c = ($guitar_hero_cheats [<index>].name_text)
			FormatText TextName = text "%c, %d" c = <text> d = ($Cheat_Hyperspeed)
			SetScreenElementProps id = <id> text = <text>
		endif
	endif
...
script toggle_cheat 
	if ($<cheat> > 0)
		...
		SetScreenElementProps id = <id> text = "gesperrt"
	endif
	show_cheat_warning
endscript
