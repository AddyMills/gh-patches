...
script translate_server_checksums_to_strings 
	...
	if GotParam \{game_mode_checksum}
		switch (<game_mode_checksum>)
			case p2_battle
			<game_mode_value> = "SCHLACHT"
			case p2_faceoff
			<game_mode_value> = "DUELL"
			case p2_pro_faceoff
			<game_mode_value> = "PRO DUELL"
			if GotParam \{difficulty_checksum}
				switch (<difficulty_checksum>)
					case easy
					<game_mode_value> = "PRO DUELL LEICHT"
					case medium
					<game_mode_value> = "PRO DUELL MITTEL"
					case hard
					<game_mode_value> = "PRO DUELL SCHWER"
					case expert
					<game_mode_value> = "PRO DUELL PROFI"
				endswitch
			endif
			case p2_coop
			<game_mode_value> = "KOOP"
		endswitch
	endif
