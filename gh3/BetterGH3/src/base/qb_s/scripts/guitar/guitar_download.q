...
script Downloads_EnumContent 
	...
	if IsEnumContentFilesDamaged
		...
		create_popup_warning_menu \{create_popup_warning_menu
			textblock = {
				text = "Un paquete de contenidos parece dañado o ilegible. Vuelve a cargar el paquete de contenidos."
				Wait
				3
				seconds
			}
...
script create_download_scan_menu 
	...
	switch <platform>
		...
		create_popup_warning_menu \{textblock = {
				text = "Comprobando el disco duro. No apagues el sistema."
			}}
		Wait \{1
			gameframes}
		case xenon
		create_popup_warning_menu \{textblock = {
				text = "Comprobando contenido descargable."
			}}
	endswitch
...
script DownloadContentLost 
	...
	printscriptinfo \{"DescargaContenidoPerdido"}
...
script DownloadContentLost_Spawned 
	...
	startrendering
	printf \{"DescargaContenidoPerdido"}
endscript

script create_downloadcontentlost_menu 
	destroy_popup_warning_menu
	create_popup_warning_menu \{title = "CONTENIDO CAMBIADO"
		title_props = {
			scale = 1.0
		}
		textblock = {
			text = "La descarga de contenido ha cambiado. Como resultado, el juego se ha reiniciado."
			pos = (640.0, 380.0)
		}
		menu_pos = (640.0, 490.0)
		dialog_dims = (288.0, 64.0)
...
script net_match_send_available_items 
	...
	if NOT IsHost
		destroy_popup_warning_menu
		create_popup_warning_menu \{title = "EN LÍNEA"
			title_props = {
				scale = 1.0
			}
			textblock = {
				text = "Enviando información de perfil. Espera por favor."
				pos = (640.0, 380.0)
			}}
	endif
