...
script create_main_menu_backdrop 
	...
	FormatText TextName = version_string_display "VERSIÓN %s" s = <version_string>
...
script pop_in_new_downloads_notifier \{time = 0.5}
	...
	pos = (100.0, 390.0)
	text = "¡NUEVA DESCARGA DE CONTENIDO!"
	CreateScreenElement {
		type = TextElement
		parent = main_menu_text_container
		text = <text>
...
script create_pause_menu \{player = 1
	...
	if ($is_network_game = 0)
		...
		if GotParam \{banner_text}
			...
			if (<for_options> = 0)
				if (<for_practice> = 1)
					<pause_player_text> = "EN PAUSA"
				else
					if NOT isSinglePlayerGame
						FormatText TextName = pause_player_text "J%d PAUSA" d = <player>
					else
						<pause_player_text> = "EN PAUSA"
					endif
				endif
	...
	if (<for_options> = 0)
		...
		CreateScreenElement {
			...
			scale = <text_scale>
			rgba = [210 130 0 250]
			text = "CALIBRAR RETRASO DE LA IMAGEN"
		...
		CreateScreenElement {
			...
			scale = <text_scale>
			rgba = [210 130 0 250]
			text = "CALIBRAR RETRASO DEL SONIDO"
		...
		if isSinglePlayerGame
			lefty_flip_text = "CAMBIAR A JUGADOR ZURDO:"
		else
			if (<player> = 1)
				lefty_flip_text = "CAMB. ZURDO J1"
			else
				lefty_flip_text = "CAMB. ZURDO J2"
			endif
		endif
		CreateScreenElement {
			type = ContainerElement
	...
	if ($is_network_game = 0)
		if NOT isSinglePlayerGame
			if (<for_practice> = 0)
				FormatText TextName = player_paused_text "JUGADOR %d EN PAUSA. SOLO OPCIONES DEL JUGADOR %d DISPONIBLES." d = <player>
				displaySprite {
					parent = pause_menu_frame_container
...
script get_diff_completion_text \{for_p2_career = 0}
	...
	if NOT (<for_p2_career>)
		FormatText TextName = diff_completion_string "%a DE %b TEMAS" a = <diff_songs_completed> b = <diff_num_songs>
		SetArrayElement ArrayName = diff_completion_text index = (<diff_index>) newvalue = (<diff_completion_string>)
	else
		FormatText TextName = diff_completion_string "%a de %b temas completados." a = <diff_songs_completed> b = <diff_num_songs>
		SetArrayElement ArrayName = diff_completion_text index = (<diff_index>) newvalue = (<diff_completion_string>)
	endif
	progression_push_current
