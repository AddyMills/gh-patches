...
script create_whammy_bar_calibration_menu \{controller = 0
	...
	<text_1> = "Pulsa la barra de vibrato completamente hasta abajo y déjala que vuelva dulcemente a su posición normal."
	button_color = "Verde"
	GetEnterButtonAssignment
	if (<assignment> = circle)
		button_color = "Rojo"
	endif
	FormatText TextName = text_2 "Pulsa el Botón %a para calibrar en esta posición." a = <button_color>
	<text_3> = "Repite la operación hasta que veas el mensaje \\c1'Posición normal calibrada'\\c0cada vez que pongas la barra de vibrato en posición normal."
	CreateScreenElement {
	...
	CreateScreenElement {
		...
		pos = (810.0, 408.0)
		text = "POSICIÓN NORMAL CALIBRADA"
		just = [center top]
	...
	add_user_control_helper text = "SELECCIONAR" button = green z = (<z> + 100)
	add_user_control_helper text = "VOLVER" button = red z = (<z> + 100)
endscript
...
script create_star_power_trigger_calibration_menu \{controller = 0
	...
	FormatText TextName = element_text "Levanta la guitarra para activar la energía Estrella y luego pulsa el botón %a para fijar este valor." a = <button_color>
	CreateScreenElement {
		type = TextBlockElement
...
script create_guitar_diagnostic_menu 
	...
	CreateScreenElement {
		<text_params>
		id = title_text
		text = "Información sobre la guitarra"
		pos = (540.0, 100.0)
	}
	CreateScreenElement {
		<text_params>
	...
	CreateScreenElement {
		<text_params>
		id = leftlength
		text = "Longitud izquierda"
		pos = (580.0, 360.0)
	}
	CreateScreenElement {
		<text_params>
		id = rightlength
		text = "Longitud derecha"
		pos = (580.0, 400.0)
	}
	CreateScreenElement {
		<text_params>
		id = lefttrigger
		text = "Gatillo izquierdo"
		pos = (580.0, 440.0)
	}
	CreateScreenElement {
		<text_params>
		id = righttrigger
		text = "Gatillo derecho"
		pos = (580.0, 480.0)
	}
	CreateScreenElement {
		<text_params>
...
script update_guitar_diagnostic_menu 
	...
	if GuitarGetAnalogueInfo \{controller = 0}
		FormatText TextName = leftxtext "X izquierda - %v" v = <leftx>
		FormatText TextName = rightxtext "Posición del vibrato - %v" v = <rightx>
		FormatText TextName = leftytext "Y izquierdo - %v" v = <lefty>
		FormatText TextName = rightytext "Y derecha - %v" v = <RightY>
		FormatText TextName = leftlengthtext "Longitud izquierda - %v" v = <leftlength>
		FormatText TextName = rightlengthtext "Longitud derecha - %v" v = <rightlength>
		FormatText TextName = lefttriggertext "Gatillo izquierdo - %v" v = <lefttrigger>
		FormatText TextName = righttriggertext "Gatillo derecho - %v" v = <righttrigger>
		FormatText TextName = verticaldisttext "Orientación vertical - %v" v = <VerticalDist>
