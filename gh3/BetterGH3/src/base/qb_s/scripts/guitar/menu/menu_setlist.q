...
script create_sl_assets 
				...
				if (<complete> = 0)
					...
					if (<encore_unlocked> = 1)
						FormatText \{TextName = completeText
							"Supera el tema que pidió el público para continuar"}
					elseif (<boss_unlocked> = 1)
						FormatText \{TextName = completeText
							"Supera el tema del rey de la guitarra para continuar"}
					else
						GetGlobalTags <tiername> param = num_songs_to_progress
						FormatText TextName = completeText "Supera %d de %p temas para seguir" d = <num_songs_to_progress> p = <num_songs_unlocked>
					endif
	...
	if ((($game_mode = p1_career) || ($game_mode = p2_career)) && $is_demo_mode = 0)
		...
		FormatText TextName = total_score_text "Puntuación de Carrera: %d" d = <career_score> usecommas
		displayText {
			parent = setlist_menu
			scale = 0.7
	...
	elseif ($current_tab = tab_bonus)
		setlist_show_helperbar {
			...
			text_option1 = "REPERTORIO"
			text_option2 = "DESCARGAS"
			button_option1 = "\\b6"
			button_option2 = "\\b8"
		}
	else
...
script setlist_show_helperbar \{text_option1 = "TEMAS EXTRA"
		text_option2 = "DESCARGAS"
		button_option1 = "\\b7"
		button_option2 = "\\b8"
		spacing = 16}
	...
	<i> = (<i> + 1)
	repeat 5
	tabs_text = ["repertorio" "temas extra" "descargas"]
